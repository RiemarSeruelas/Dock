...
export default function DockFlowApp() {
  ...
  const receivingData = user.role === "ecosystem" ? { ...data, shipments: data.shipments.filter((shipment) => isEcosystemReceiving(shipment, user)) } : { ...data, shipments: data.shipments.filter((shipment) => !shipment.items.some((item) => /ecosystem/i.test(String(item.deliverySite || "")))) };
