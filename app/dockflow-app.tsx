...
import { EcosystemPage } from "./receiving-ui";
...
const EMPTY_DATA: AppData = {
  ...
  notifications: [],
  importBatches: [],
  settings: { flexibleScheduling: true, dockCount: 3, graceMinutes: 15, siteName: "Cavite Foods Receiving", siteAddress: "", siteCoordinates: null, availableDates: [], availableSlots: [] },
};
...
const NAV_ITEMS: { id: View; label: string; icon: Icon; roles?: Role[] }[] = [
  ...
  { id: "entries", label: "Delivery Entries", icon: ClipboardList, roles: ["supplier", "ecosystem"] },
  ...
  { id: "sap", label: "Receiving Records", icon: ClipboardList, roles: ["admin", "sap", "planner", "warehouse"] },
  { id: "clearance", label: "Clearance", icon: PackageCheck, roles: ["admin", "planner", "sap", "warehouse"] },
...
const ROLE_VIEWS: Record<Role, View[]> = {
  ...
  planner: ["overview", "monitoring", "schedule", "history", "reports", "ecosystem", "sap", "clearance"],
...
const STATUS_META: Record<ShipmentStatus, { label: string; color: string; step: number }> = {
  ...
  SAP_CLEARANCE: { label: "SAP Clearance", color: "violet", step: 4 },
  UNLOADING: { label: "Unloading", color: "orange", step: 5 },
  RECEIVED: { label: "Received", color: "green", step: 6 },
  GATE_OUT: { label: "Gate out", color: "teal", step: 7 },
...
const ROLE_LABELS: Record<Role, string> = {
  ...
  security: "Security",
  warehouse: "Warehouse",
  qa: "Quality inspection", ecosystem: "Ecosystem", sap: "SAP Analyst",
};
const ACCOUNT_ROLE_OPTIONS: { role: Role; label: string }[] = [
  ...
  { role: "sap", label: "SAP Analyst" },
];
const accountRoleLabel = (account: Pick<SessionUser, "role" | "workArea" | "securityScope">) => {
  if (account.role === "security") return `Security · ${account.securityScope === "ECOSYSTEM" ? "Ecosystem" : "Company"}`;
...
type EcosystemMonitoringView = "receiving" | "sending";
...
const isEcosystemReceiving = (shipment: Shipment, user: SessionUser) => Number(shipment.destinationEcosystemId) === Number(user.supplierId) || (!shipment.destinationEcosystemId && shipment.items.some((item) => /ecosystem/i.test(String(item.deliverySite || ""))));
const isEcosystemSending = (shipment: Shipment, user: SessionUser) => Number(shipment.supplierId) === Number(user.supplierId);
...
function StatusPill({ status }: { status: ShipmentStatus; receipt?: Shipment["receipt"] }) {
...
function OverviewPage({ data, user, area, canSwitchArea, ecosystemReceiving = false, onAreaChange, onOpenShipment }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; ecosystemReceiving?: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void }) {
  const today = localDate();
  const areaShipments = ecosystemReceiving ? data.shipments : data.shipments.filter(shipment => shipmentArea(shipment) === area);
  ...
  const completed = todayShipments.filter((shipment) => shipment.status === "GATE_OUT").length;
  const atSite = todayShipments.filter((shipment) => ["GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED"].includes(shipment.status)).length;
  ...
  const dockActiveShipments = areaShipments.filter((shipment) => ["GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED"].includes(shipment.status) && shipment.dock);
  ...
  const nextAction = areaShipments.find((shipment) => {
    ...
    if (user.role === "security") return ["IN_TRANSIT", "UNLOADING", "RECEIVED"].includes(shipment.status);
  ...
  return <div className="page-stack overview-page">
    <section className="hero-row">
      <div><span className="eyebrow">{formatDate(today)} · Live operations</span><h1>Good day, {user.name.split(" ")[0]}.</h1></div>
      <div className="hero-actions">{ecosystemReceiving ? <span className="area-scope-chip ecosystem-scope-chip receiving">Supplier → Ecosystem</span> : canSwitchArea ? <AreaToggle value={area} onChange={onAreaChange}/> : <span className="area-scope-chip">{area === "DRESSINGS" ? "Dressings" : "Savoury"}</span>}</div>
    </section>
    ...
    <section className="metrics-grid">
      ...
      <MetricCard label="Gate-out complete" value={`${completed}/${todayShipments.length}`} icon={PackageCheck} tone="green" />
...
const SCAN_STATIONS: Record<ScanStage, { label: string; helper: string; icon: Icon; roles: Role[] }> = {
  ...
  SAP_CLEARANCE: { label: "SAP Clearance", helper: "Record SAP document clearance", icon: ClipboardList, roles: ["admin", "sap"] },
  UNLOADING: { label: "Unloading", helper: "Record unloading start", icon: Boxes, roles: ["admin", "warehouse", "ecosystem"] },
  RECEIVED: { label: "Received", helper: "Record receiving completion", icon: PackageCheck, roles: ["admin", "warehouse", "ecosystem"] },
};
...
function OperationsPage({ data, user, onScanStage, onGateReject, onOpenShipment }: { data: AppData; user: SessionUser; onScanStage: (scanValue: string, stage: ScanStage, options?: { gateDecision?: "ACCEPT" }) => Promise<Shipment>; onGateReject: (shipment: Shipment, reasonCode: string, otherReason?: string) => Promise<Shipment>; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const visibleShipments = user.role === "ecosystem"
    ? stage === "LOOKUP"
      ? approvedShipments
      : stage === "TRIP"
        ? approvedShipments.filter((shipment) => isEcosystemSending(shipment, user))
        : approvedShipments.filter((shipment) => isEcosystemReceiving(shipment, user))
    : user.role === "supplier"
      ? approvedShipments.filter(shipment => Number(shipment.supplierId) === Number(user.supplierId))
      : approvedShipments;
  const stageStatuses: Record<ScanStage, ShipmentStatus[]> = { LOOKUP: ["BOOKED", "IN_TRANSIT", "GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED", "GATE_OUT"], TRIP: ["BOOKED"], GATE: ["BOOKED", "IN_TRANSIT", "RECEIVED"], SAP_CLEARANCE: ["GATE_IN"], UNLOADING: ["SAP_CLEARANCE"], RECEIVED: ["UNLOADING"] };
  ...
  const gateIsExit = stage === "GATE" && Boolean(selected && ["RECEIVED", "GATE_OUT"].includes(selected.status));
  ...
  return <div className="page-stack operations-page">
    ...
    {availableStages.length > 0 && <><section className="operations-grid">
      ...
      <article className={`panel verification-panel ${selected ? "has-selection" : "empty-selection"}`}>
        {!selected ? <EmptyState icon={QrCode} title={`Ready for the ${activeStation.label} scan`} body="Scan the printed booking QR. The entry status updates as soon as the code is accepted." /> : <>
          ...
          <div className="identity-strip"><div className="truck-tile large"><Truck size={26} /></div><div><strong>{selected.supplier}</strong><span>{selected.truckPlate}</span></div><button className="text-button" onClick={() => onOpenShipment(selected)}>View all details</button></div>
...
function EntriesPage({ data, onOpenShipment }: { data: AppData; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const isArchived = (shipment: Shipment) => shipment.status === "GATE_OUT";
...
function ReportsPage({ data, user, token, onOpenShipment }: { data: AppData; user: SessionUser; token: string; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const [reportArea, setReportArea] = useState<"ALL" | ReceivingArea>(user.role === "supplier" && user.workAreas?.length === 1 ? user.workAreas[0] : "ALL");
  ...
  const reportableShipments = data.shipments.filter((shipment) => shipment.scheduledDate.startsWith(reportMonth) && shipment.bookingStatus === "APPROVED" && shipment.status !== "REJECTED" && (reportArea === "ALL" || shipment.items.some(item => String(item.deliverySite || "").toUpperCase() === reportArea)));
  ...
  const inProgress = filteredShipments.filter((shipment) => !["BOOKED", "GATE_OUT"].includes(shipment.status)).length;
  const supplierRows = Array.from(new Set(filteredShipments.map((shipment) => shipment.supplier))).map((supplier) => { const rows = filteredShipments.filter((shipment) => shipment.supplier === supplier); return { supplier, rows, deliveries: rows.length, completed: rows.filter((shipment) => shipment.status === "GATE_OUT").length, trip: average(rows.map((shipment) => elapsed(shipment.gateInAt, shipment.unloadingAt))), unload: average(rows.map((shipment) => elapsed(shipment.unloadingAt, shipment.gateOutAt))), turnaround: average(rows.map((shipment) => elapsed(shipment.gateInAt, shipment.gateOutAt))) }; });
  const reportDetail = supplierRows.find((row) => row.supplier === selectedSupplier) || null;
  const exportReport = async () => {
    if (exporting) return;
    setExporting(true);
    try {
      const query = `?month=${reportMonth}${supplierFilter === "ALL" ? "" : `&supplierId=${encodeURIComponent(supplierFilter)}`}${reportArea === "ALL" ? "" : `&area=${reportArea}`}`;
  ...
  return <div className="page-stack reports-page">
    <section className="hero-row"><div><span className="eyebrow">{user.role === "supplier" ? "My supplier account" : "Company supplier scorecard"}</span><h1>{user.role === "supplier" ? "My delivery performance" : "Supplier performance"}</h1></div><div className="report-controls"><input className="report-month-input" type="month" aria-label="Report month" value={reportMonth} onChange={(event) => setReportMonth(event.target.value)} />{(user.role !== "supplier" || (user.workAreas?.length || 0) > 1) && <label><span>Area</span><select value={reportArea} onChange={event => setReportArea(event.target.value as "ALL" | ReceivingArea)}><option value="ALL">All areas</option>{(user.role !== "supplier" || user.workAreas?.includes("DRESSINGS")) && <option value="DRESSINGS">Dressings</option>}{(user.role !== "supplier" || user.workAreas?.includes("SAVOURY")) && <option value="SAVOURY">Savoury</option>}</select></label>}{user.role !== "supplier" && <label><span>Supplier account</span><select value={supplierFilter} onChange={(event) => setSupplierFilter(event.target.value)}><option value="ALL">View all suppliers</option>{data.suppliers.map((supplier) => <option value={supplier.id} key={supplier.id}>{supplier.name}</option>)}</select></label>}<button className="button secondary" disabled={exporting} onClick={() => void exportReport()}>{exporting ? <Loader2 className="spin" size={17} /> : <Download size={17} />} {exporting ? "Building workbook" : "Export Performance"}</button></div></section>
    <section className="metrics-grid report-metrics"><MetricCard label="Confirmed deliveries" value={total} helper="Supplier-confirmed bookings" icon={ShieldCheck} tone="blue" /><MetricCard label="In progress" value={inProgress} helper="Active delivery journeys" icon={Truck} tone="teal" /><MetricCard label="Gate-out complete" value={completed} helper="Full journey completed" icon={PackageCheck} tone="green" /><MetricCard label="Average unloading" value={`${average(filteredShipments.map((shipment) => elapsed(shipment.unloadingAt, shipment.gateOutAt)))} min`} helper="Unload to gate out" icon={Gauge} tone="orange" /><MetricCard label="Average site time" value={`${average(filteredShipments.map((shipment) => elapsed(shipment.gateInAt, shipment.gateOutAt)))} min`} helper="Gate in to gate out" icon={Clock3} tone="violet" /></section>
...
type ManualRequestItem = { materialCode: string; description: string; quantity: string; uom: string; poNumber: string };
function ManualRequestModal({ data, user, onClose, onSubmit }: { data: AppData; user: SessionUser; onClose: () => void; onSubmit: (payload: { supplierId?: number; area: ReceivingArea; scheduledDate: string; scheduledTime: string; notes: string; items: ManualRequestItem[] }) => Promise<void> }) {
  const allowedAreas = user.workArea === "DRESSINGS" || user.workArea === "SAVOURY" ? [user.workArea] : user.role === "supplier" && user.workAreas?.length ? user.workAreas : ["DRESSINGS", "SAVOURY"];
  const [supplierId, setSupplierId] = useState(Number(user.supplierId || data.suppliers[0]?.id || 0));
  const [area, setArea] = useState<ReceivingArea>((allowedAreas[0] || "DRESSINGS") as ReceivingArea);
  const [scheduledDate, setScheduledDate] = useState(localDate());
  const [scheduledTime, setScheduledTime] = useState("09:00");
  const [notes, setNotes] = useState("");
  const [items, setItems] = useState<ManualRequestItem[]>([{ materialCode: "", description: "", quantity: "", uom: "KG", poNumber: "" }]);
  const [busy, setBusy] = useState(false), [error, setError] = useState("");
  const updateItem = (index: number, patch: Partial<ManualRequestItem>) => setItems(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item));
  return <Modal title="Request delivery" subtitle="Create a delivery request directly without uploading an SDS file." onClose={onClose} wide><form className="modal-form manual-request-form" onSubmit={async event => { event.preventDefault(); if (busy) return; setBusy(true); setError(""); try { await onSubmit({ supplierId: user.role === "supplier" ? undefined : supplierId, area, scheduledDate, scheduledTime, notes, items }); onClose(); } catch (reason) { setError(reason instanceof Error ? reason.message : "The request could not be created."); } finally { setBusy(false); } }}>
    <div className="form-grid">{user.role !== "supplier" && <label>Supplier<select value={supplierId} onChange={event => setSupplierId(Number(event.target.value))} required><option value="">Choose supplier</option>{data.suppliers.map(supplier => <option value={supplier.id} key={supplier.id}>{supplier.name}</option>)}</select></label>}<label>Receiving area<select value={area} onChange={event => setArea(event.target.value as ReceivingArea)}>{allowedAreas.map(value => <option value={value} key={value}>{value === "DRESSINGS" ? "Cavite Foods Dressings" : "Cavite Foods Savoury"}</option>)}</select></label><label>Delivery date<input type="date" value={scheduledDate} onChange={event => setScheduledDate(event.target.value)} required /></label><label>Entrance time<input type="time" value={scheduledTime} onChange={event => setScheduledTime(event.target.value)} required /></label></div>
    <section className="manual-request-items"><header><div><h3>Materials</h3><small>Add the same details normally read from the SDS.</small></div><button type="button" className="button secondary compact" onClick={() => setItems(current => [...current, { materialCode: "", description: "", quantity: "", uom: "KG", poNumber: "" }])}><Plus size={15}/> Add material</button></header>{items.map((item, index) => <div className="manual-request-item" key={index}><input aria-label={`Material code ${index + 1}`} placeholder="Material code" value={item.materialCode} onChange={event => updateItem(index, { materialCode: event.target.value })} required/><input aria-label={`Description ${index + 1}`} placeholder="Material description" value={item.description} onChange={event => updateItem(index, { description: event.target.value })} required/><input aria-label={`Quantity ${index + 1}`} type="number" min="0.000001" step="any" placeholder="Quantity" value={item.quantity} onChange={event => updateItem(index, { quantity: event.target.value })} required/><input aria-label={`UOM ${index + 1}`} placeholder="UOM" value={item.uom} onChange={event => updateItem(index, { uom: event.target.value })} required/><input aria-label={`PO number ${index + 1}`} placeholder="PO number (optional)" value={item.poNumber} onChange={event => updateItem(index, { poNumber: event.target.value })}/><button type="button" className="icon-button danger" aria-label={`Remove material ${index + 1}`} disabled={items.length === 1} onClick={() => setItems(current => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={16}/></button></div>)}</section>
    <label>Notes (optional)<textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder="Add instructions for the supplier or company" /></label>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy || (user.role !== "supplier" && !supplierId)}>{busy ? <Loader2 className="spin" size={17}/> : <Plus size={17}/>} Send request</button></div>
  </form></Modal>;
}

type AccountForm = Record<string, string | number | string[]>;
function AdminCreateModal({ currentUser, onClose, onSubmit }: { currentUser: SessionUser; onClose: () => void; onSubmit: (form: AccountForm) => Promise<void> | void }) {
  const [form, setForm] = useState<AccountForm>({ name: "", username: "", email: "", password: "", role: "supplier", workArea: currentUser.workArea || "DRESSINGS", workAreas: currentUser.workArea ? [currentUser.workArea] : ["DRESSINGS"], securityScope: "COMPANY" });
  const set = (name: string, value: string | number | string[]) => setForm({ ...form, [name]: value });
  const [busy,setBusy]=useState(false),[error,setError]=useState("");
  const [passwordVisible,setPasswordVisible]=useState(false);
  const needsEmail = true;
  const role = String(form.role) as Role;
  const areaRequired = ["planner", "warehouse", "sap"].includes(role);
  ...
  const supplierAreas = Array.isArray(form.workAreas) ? form.workAreas : [];
  const toggleSupplierArea = (area: string) => set("workAreas", supplierAreas.includes(area) ? supplierAreas.filter(value => value !== area) : [...supplierAreas, area]);
  return <Modal title="Add user account" onClose={onClose}>
    <form className="modal-form" onSubmit={async event => { event.preventDefault(); if (busy) return; setBusy(true); setError(""); try { await onSubmit(form); } catch (error) { setError(error instanceof Error ? error.message : "Unable to add account"); } finally { setBusy(false); } }}>
      <div className="form-grid">
        <label>Account display name<input value={form.name} onChange={event => set("name", event.target.value)} required /></label>
        <label>Username<input value={form.username} onChange={event => set("username", event.target.value.toLowerCase())} required /></label>
        {needsEmail && <label>Email<input type="email" value={form.email} onChange={event => set("email", event.target.value.toLowerCase())} required /></label>}
        <label>Initial password<span className="activation-password admin-create-password" onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPasswordVisible(false); }}><input type={passwordVisible ? "text" : "password"} minLength={8} value={form.password} onChange={event => set("password", event.target.value)} required /><button type="button" onClick={() => setPasswordVisible(current => !current)} aria-label={passwordVisible ? "Hide password" : "Show password"}>{passwordVisible ? <EyeOff size={17}/> : <Eye size={17}/>}</button></span></label>
        <label>Account role<select value={form.role} onChange={event => { const nextRole = event.target.value; setForm(current => ({ ...current, role: nextRole, workArea: nextRole === "supplier" ? "" : currentUser.workArea || (nextRole === "admin" ? "" : "DRESSINGS"), workAreas: currentUser.workArea ? [currentUser.workArea] : ["DRESSINGS"], securityScope: nextRole === "security" ? "COMPANY" : current.securityScope })); }}>{roleOptions.map(({ role: optionRole, label }) => <option value={optionRole} key={optionRole}>{label}</option>)}</select></label>
        {role === "supplier" && <fieldset className="supplier-area-checklist"><legend>Supplier access</legend>{["DRESSINGS", "SAVOURY"].map(area => <label key={area}><input type="checkbox" checked={supplierAreas.includes(area)} disabled={Boolean(currentUser.workArea && currentUser.workArea !== area)} onChange={() => toggleSupplierArea(area)} />{area === "DRESSINGS" ? "Dressings" : "Savoury"}</label>)}<small>Select both when this supplier delivers to both areas.</small></fieldset>}
        {role === "security" && <label>Security access<select value={form.securityScope} onChange={event => set("securityScope", event.target.value)}><option value="COMPANY">Company deliveries</option><option value="ECOSYSTEM">Ecosystem deliveries</option></select></label>}
        {areaChoice && <label>Access area{currentUser.workArea ? <span className="locked-access-area"><ShieldCheck size={17}/><span><b>{currentUser.workArea === "DRESSINGS" ? "Dressings" : "Savoury"}</b><small>Locked to your administrator area</small></span></span> : <select value={form.workArea} onChange={event => set("workArea", event.target.value as WorkArea)} required={areaRequired}>{role === "admin" && <option value="">Both</option>}<option value="DRESSINGS">Dressings</option><option value="SAVOURY">Savoury</option></select>}</label>}
      </div>
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy || (role === "supplier" && supplierAreas.length === 0)}><Plus size={17}/> Add account</button></div>
    </form>
  </Modal>;
}
...
function ReceivingSitePanel({ onUnlock, onSave }: { onUnlock: (password: string) => Promise<{ siteAddress: string; configured: boolean }>; onSave: (address: string, mapReference: string, password: string) => Promise<void> }) {
  const [password, setPassword] = useState(""), [siteAddress, setSiteAddress] = useState(""), [unlocked, setUnlocked] = useState(false), [busy, setBusy] = useState(false), [error, setError] = useState("");
...
function AdminPage({ data, token, currentUser, onAddUser, onDeleteUser, onUnlockSiteAddress, onSaveSiteAddress, onCalculateSupplierEta }: { data: AppData; token: string; currentUser: SessionUser; onAddUser: (form: AccountForm) => Promise<void> | void; onDeleteUser: (account: SessionUser, password: string) => Promise<void> | void; onUnlockSiteAddress: (password: string) => Promise<{ siteAddress: string; configured: boolean }>; onSaveSiteAddress: (address: string, mapReference: string, password: string) => Promise<void>; onCalculateSupplierEta: (supplier: SupplierAccount, address: string, mapReference: string) => Promise<void> | void }) {
  ...
  const finishCreate = async (form: AccountForm) => { await onAddUser(form); setCreating(false); };
  const matchingAccounts = data.users.filter(user => `${user.name} ${user.username} ${user.email || ""}`.toLowerCase().includes(search.toLowerCase()));
  const accountGroups = [
    { key: "SUPPLIERS", title: "Suppliers", helper: "One account per supplier across all receiving areas", accounts: matchingAccounts.filter(account => account.role === "supplier") },
    { key: "BOTH", title: "Cavite Foods", helper: "", accounts: matchingAccounts.filter(account => account.role !== "supplier" && (!account.workArea || account.workArea === "ECOSYSTEM")) },
    { key: "DRESSINGS", title: "Cavite Foods Dressings", helper: "", accounts: matchingAccounts.filter(account => account.role !== "supplier" && account.workArea === "DRESSINGS") },
    { key: "SAVOURY", title: "Cavite Foods Savoury", helper: "", accounts: matchingAccounts.filter(account => account.role !== "supplier" && account.workArea === "SAVOURY") },
  ].filter((group) => !currentUser.workArea || group.key === "SUPPLIERS" || group.key === currentUser.workArea);
...
function ShipmentModal({ shipment, token, user, onClose, onDownloadPdf, onUpdateSupplierDetails }: { shipment: Shipment; token: string; user: SessionUser; onClose: () => void; onDownloadPdf: (shipment: Shipment) => Promise<void> | void; onUpdateSupplierDetails: (shipment: Shipment, details: Record<string, string>) => Promise<void> }) {
  ...
  const stages = [
    ...
    { label: "SAP Clearance", status: "SAP_CLEARANCE" as ShipmentStatus, at: shipment.sapClearanceAt },
  ...
  return <Modal className="booking-receipt-modal" title={shipment.shipmentNumber} subtitle={`${shipment.bookingReceipt} · ${shipment.supplier}`} onClose={onClose} wide>
    <div className="shipment-detail">
      ...
      <dl className="detail-grid">{user.role !== "supplier" && <div><dt>Delivery code</dt><dd>{shipment.deliveryCode || shipment.confirmedTruckLoads?.[0]?.deliveryCode || "Waiting for confirmation"}</dd></div>}<div><dt>Delivery date</dt><dd>{formatDate(shipment.scheduledDate)}</dd></div><div><dt>Entrance time</dt><dd>{shipment.scheduledTime}</dd></div><div><dt>Site</dt><dd>{deliverySite}</dd></div><div><dt>Gate in</dt><dd>{formatTime(shipment.gateInAt)}</dd></div>{user.role !== "supplier" && <div><dt>Arrival classification</dt><dd>{shipment.arrivalClassification?<span className={`arrival-classification ${shipment.arrivalClassification.toLowerCase()}`}>{shipment.arrivalClassification.replace('_',' ')}</span>:"Awaiting Gate In"}</dd></div>}<div><dt>PO number</dt><dd>{shipment.items.map(item => item.poNumber).filter(Boolean).join(", ") || shipment.poNumber || "—"}</dd></div><div><dt>DR number</dt><dd>{shipment.drNumber || "—"}</dd></div><div><dt>Helpers</dt><dd>{[shipment.helper1Name, shipment.helper2Name].filter(Boolean).join(", ") || "—"}</dd></div></dl>
...
export default function DockFlowApp() {
  ...
  const handleLogin = (nextUser: SessionUser, nextToken: string, verificationNotice?: string | null) => {
    ...
    setEcosystemMonitoringView("receiving");
    setActivationNotice(verificationNotice || "");
    const initialArea = nextUser.workArea || nextUser.workAreas?.[0];
    if (initialArea === "SAVOURY" || initialArea === "DRESSINGS") setReceivingArea(initialArea);
  ...
  const scanShipmentStage = async (scanValue: string, stage: ScanStage, options?: { gateDecision?: "ACCEPT" }) => {
    ...
    const result = await apiRequest<{ shipment: Shipment; message: string }>(token, "/api/shipments/scan-stage", "POST", { scanValue, stage, ...options });
    await refresh();
  ...
  const createManualRequest = async (payload: { supplierId?: number; area: ReceivingArea; scheduledDate: string; scheduledTime: string; notes: string; items: ManualRequestItem[] }) => {
    const result = await apiRequest<{ shipment: Shipment; notified: number }>(token, "/api/delivery-requests", "POST", payload);
    await refresh();
    notify(`Request created for ${result.shipment.supplier}. ${result.notified} account${result.notified === 1 ? "" : "s"} notified.`);
  };
  ...
  const addUser = async (form: AccountForm) => { await apiRequest(token,"/api/users","POST",form); await refresh(); notify(`${form.name} added. Their verification code will be sent on first login.`); };
  ...
  const saveSiteAddress = async (address: string, mapReference: string, password: string) => {
    try {
      ...
      await refresh();
      notify(result.geocoded ? `Address saved and matched${result.matchedAddress ? ` to ${result.matchedAddress}` : ""}. Recalculate each supplier route.` : result.warning || "Address saved. Use a more complete address before calculating ETA.");
    } catch (reason) {
  ...
  const ecosystemSecurity = user.role === "security" && user.securityScope === "ECOSYSTEM";
  const receivingData = user.role === "ecosystem" ? { ...data, shipments: data.shipments.filter((shipment) => isEcosystemReceiving(shipment, user)) } : ecosystemSecurity ? data : { ...data, shipments: data.shipments.filter((shipment) => !shipment.items.some((item) => /ecosystem/i.test(String(item.deliverySite || "")))) };
  ...
  const monitoringData = user.role === "ecosystem" ? (ecosystemMonitoringView === "receiving" ? receivingData : outgoingData) : receivingData;
  const supplierAreas = user.workAreas || [];
  const canSwitchArea = (user.role === "security" && !ecosystemSecurity) || (user.role === "admin" && !user.workArea) || (user.role === "supplier" && supplierAreas.length > 1);
  const singleSupplierArea = user.role === "supplier" && supplierAreas.length === 1 ? supplierAreas[0] : null;
  const activeReceivingArea: ReceivingArea = user.workArea === "DRESSINGS" || user.workArea === "SAVOURY" ? user.workArea : singleSupplierArea || receivingArea;
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className={`sidebar ${sidebarCollapsed ? "collapsed" : ""} ${mobileNav ? "open" : ""}`}><div className="sidebar-top"><div className="brand-lockup brand-light"><span className="brand-mark"><Route size={22} /></span><span><b>DockFlow</b></span></div><button className="sidebar-collapse" onClick={() => setSidebarCollapsed((current) => !current)} aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>{sidebarCollapsed ? <ChevronsRight size={20} /> : <ChevronsLeft size={20} />}</button><button className="mobile-close" onClick={() => setMobileNav(false)}><X size={20} /></button></div><nav>{visibleNav.map(item => { const NavIcon = item.icon; if (item.id === "monitoring" && user.role === "ecosystem") return <div className="sidebar-nav-group" key={item.id}><button className={view === item.id ? "active" : ""} title={sidebarCollapsed ? item.label : undefined} onClick={() => { setView(item.id); setMobileNav(false); }}><span><NavIcon size={19} /></span><div><b>{item.label}</b></div></button><div className="sidebar-subnav" aria-label="Ecosystem monitoring views"><button className={view === "monitoring" && ecosystemMonitoringView === "receiving" ? "active" : ""} onClick={() => { setEcosystemMonitoringView("receiving"); setView("monitoring"); setMobileNav(false); }}><PackageCheck size={15}/><b>Receiving</b></button><button className={view === "monitoring" && ecosystemMonitoringView === "sending" ? "active" : ""} onClick={() => { setEcosystemMonitoringView("sending"); setView("monitoring"); setMobileNav(false); }}><Truck size={15}/><b>Sending</b></button></div></div>; return <button className={view === item.id ? "active" : ""} title={sidebarCollapsed ? item.label : undefined} key={item.id} onClick={() => selectNavigation(item.id)}><span><NavIcon size={19} /></span><div><b>{item.label}</b></div></button>; })}</nav><div className="sidebar-user"><span className="user-avatar">{initials(user.name)}</span><div><b>{user.name}</b><small>{accountRoleLabel(user)}</small></div><button onClick={logout} title="Sign out"><LogOut size={17} /></button></div></aside>
    ...
    {selectedShipment && <ShipmentModal shipment={selectedShipment} token={token} user={user} onClose={() => setSelectedShipment(null)} onDownloadPdf={downloadBookingPdf} onUpdateSupplierDetails={updateSupplierDetails} />}{excelImport && <ExcelImportModal token={token} onClose={() => setExcelImport(false)} onImported={imported} />}{manualRequestOpen && <ManualRequestModal data={data} user={user} onClose={() => setManualRequestOpen(false)} onSubmit={createManualRequest} />}{selfVerificationOpen && <VerifyAccountEmailModal account={user} onClose={() => setSelfVerificationOpen(false)} onSaveEmail={saveAccountEmail} onSendCode={sendEmailCode} onVerify={verifyEmail} />}
  </div>;
}
