...
import { EcosystemPage, KpiPanel, ReceiptModal, SapPage } from "./receiving-ui";
...
type View = "overview" | "monitoring" | "schedule" | "entries" | "operations" | "history" | "reports" | "admin" | "sap" | "ecosystem";
...
const NAV_ITEMS: { id: View; label: string; icon: Icon; roles?: Role[] }[] = [
  ...
  { id: "sap", label: "SAP register", icon: ClipboardList, roles: ["sap"] },
...
const ROLE_VIEWS: Record<Role, View[]> = {
  admin: ["overview", "monitoring", "schedule", "operations", "history", "reports", "admin", "ecosystem"],
  planner: ["overview", "monitoring", "schedule", "history", "reports", "ecosystem"],
  ...
  ecosystem: ["schedule", "entries", "monitoring", "operations", "history", "reports", "ecosystem"],
  sap: ["sap"],
};
...
const ACCOUNT_ROLE_OPTIONS: { role: Role; label: string }[] = [
  ...
  { role: "qa", label: "Quality inspection" },
...
const accountRoleLabel = (account: Pick<SessionUser, "role" | "workArea">) => {
  if (["planner", "warehouse", "qa"].includes(account.role) && account.workArea) return `${ROLE_LABELS[account.role]} ${account.workArea === "DRESSINGS" ? "Dressings" : account.workArea === "ECOSYSTEM" ? "Ecosystem" : "Savoury"}`;
...
function StatusPill({ status, receipt }: { status: ShipmentStatus; receipt?: Shipment["receipt"] }) {
  ...
  return <span className={`status-pill status-${meta.color}`}><span />{meta.label}{receipt?.inFull === false ? " · Not in Full" : ""}</span>;
}
...
const SCAN_STATIONS: Record<ScanStage, { label: string; helper: string; icon: Icon; roles: Role[] }> = {
  ...
  UNLOADING: { label: "Unloading", helper: "Record unloading start", icon: Boxes, roles: ["admin", "warehouse", "qa", "ecosystem"] },
  RECEIVED: { label: "Received", helper: "Complete delivery", icon: PackageCheck, roles: ["admin", "warehouse", "qa", "ecosystem"] },
};
...
function OperationsPage({ data, user, onScanStage, onOpenShipment }: { data: AppData; user: SessionUser; onScanStage: (scanValue: string, stage: ScanStage, receipt?: ReceiptInput) => Promise<Shipment>; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const visibleShipments = user.role === "ecosystem" && stage !== "LOOKUP" ? approvedShipments.filter(shipment => Number(shipment.destinationEcosystemId) === Number(user.supplierId)) : user.role === "supplier" ? approvedShipments.filter(shipment => Number(shipment.supplierId) === Number(user.supplierId)) : approvedShipments;
...
function ReportsPage({ data, user, token, onOpenShipment }: { data: AppData; user: SessionUser; token: string; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  return <div className="page-stack reports-page"><KpiPanel token={token} supplierId={supplierFilter} />
...
function AdminCreateModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (form: Record<string, string | number>) => void }) {
  ...
  const needsEmail = ["planner", "supplier", "ecosystem"].includes(String(form.role));
  return <Modal title="Add user account" onClose={onClose}><form className="modal-form" onSubmit={event => { event.preventDefault(); onSubmit({ ...form, email: needsEmail ? form.email : "" }); }}><div className="form-grid"><label>Account display name<input value={form.name} onChange={event => set("name", event.target.value)} required /></label><label>Username<input value={form.username} onChange={event => set("username", event.target.value.toLowerCase())} required /></label>{needsEmail && <label>Email<input type="email" value={form.email} onChange={event => set("email", event.target.value.toLowerCase())} required /></label>}<label>Initial password<input type="password" minLength={8} value={form.password} onChange={event => set("password", event.target.value)} required /></label><label>Account role<select value={form.role} onChange={event => set("role", event.target.value)}>{ACCOUNT_ROLE_OPTIONS.map(({ role, label }) => <option value={role} key={role}>{label}</option>)}</select></label>{["planner", "warehouse", "qa"].includes(String(form.role)) && <label>Work area<select value={form.workArea} onChange={event => set("workArea", event.target.value as WorkArea)} required><option value="DRESSINGS">Dressings</option><option value="SAVOURY">Savoury</option>{form.role !== "planner" && <option value="ECOSYSTEM">Ecosystem</option>}</select></label>}</div><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary"><Plus size={17} /> Add account</button></div></form></Modal>;
}
...
function ReceivingSiteModal({ onClose, onUnlock, onSave }: { onClose: () => void; onUnlock: (password: string) => Promise<{ siteAddress: string; configured: boolean }>; onSave: (address: string, mapReference: string, password: string) => Promise<void> }) {
  ...
  const save = async (event: React.FormEvent) => { event.preventDefault(); if (busy) return; setBusy(true); setError(""); try { await onSave(siteAddress, mapReference, password); onClose(); } catch (reason) { setError(reason instanceof Error ? reason.message : "The receiving site could not be saved."); setBusy(false); } };
  return <Modal title="Receiving site" subtitle={unlocked ? "The exact destination is visible only after administrator-password confirmation." : "Confirm the current administrator password to view or change the protected destination."} onClose={onClose}>{!unlocked ? <form className="modal-form" onSubmit={unlock}><label>Administrator password<input autoFocus type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>{error && <div className="form-error"><AlertTriangle size={16} />{error}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy || !password}>{busy ? <Loader2 className="spin" size={17} /> : <ShieldCheck size={17} />} Unlock destination</button></div></form> : <form className="modal-form" onSubmit={save}><label>Receiving-site address<input autoFocus value={siteAddress} onChange={(event) => setSiteAddress(event.target.value)} placeholder="Complete receiving-site address" minLength={6} required /></label><label>Google Maps link (optional)<input value={mapReference} onChange={(event) => setMapReference(event.target.value)} placeholder="Paste a Google Maps link for an exact match" /></label>{error && <div className="form-error"><AlertTriangle size={16} />{error}</div>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy || siteAddress.trim().length < 6}>{busy ? <Loader2 className="spin" size={17} /> : <Check size={17} />} Save receiving site</button></div></form>}</Modal>;
}
...
function AdminPage({ data, currentUser, onAddUser, onDeleteUser, onUnlockSiteAddress, onSaveSiteAddress, onCalculateSupplierEta }: { data: AppData; currentUser: SessionUser; onAddUser: (form: Record<string, string | number>) => void; onDeleteUser: (account: SessionUser, password: string) => Promise<void> | void; onUnlockSiteAddress: (password: string) => Promise<{ siteAddress: string; configured: boolean }>; onSaveSiteAddress: (address: string, mapReference: string, password: string) => Promise<void>; onCalculateSupplierEta: (supplier: SupplierAccount, address: string, mapReference: string) => Promise<void> | void }) {
  ...
  const [section, setSection] = useState<"accounts" | null>(null);
  ...
  const [receivingSiteOpen, setReceivingSiteOpen] = useState(false);
  const finishCreate = (form: Record<string, string | number>) => { onAddUser(form); setCreating(false); };
  const emailAccounts = data.users.filter((account) => ["planner", "supplier", "ecosystem"].includes(account.role));
  ...
  return <div className="page-stack"><section className="hero-row"><div><span className="eyebrow">System control</span><h1>Administration</h1></div></section>
    <section className="admin-hub-actions"><button className={section === "accounts" ? "active" : ""} onClick={() => setSection("accounts")}><span className="settings-icon"><Settings size={22} /></span><span><b>Accounts</b><small>Manage access, supplier routes, and retained records</small></span><ArrowRight size={19} /></button><button onClick={() => setReceivingSiteOpen(true)}><span className="settings-icon"><MapPinned size={22} /></span><span><b>Receiving site</b><small>{data.settings.siteAddressConfigured ? "Protected destination configured" : "Destination setup required"}</small></span><ShieldCheck size={19} /></button></section>
    {section === "accounts" && <><section className="panel email-admin-card"><span className="settings-icon"><ShieldCheck size={20} /></span><div><strong>Unverified accounts</strong><small>Planner and supplier account owners complete their own verification.</small></div><div className="email-verification-list read-only">{unverified.map((account) => <div key={account.id}><span><b>{account.name}</b><small>{account.email || "No email added"}</small></span><em className="pending">Pending</em></div>)}{!unverified.length && <small>All email-enabled accounts are verified.</small>}</div></section><section className="panel admin-panel"><div className="toolbar"><label className="search-box"><Search size={17} /><input placeholder="Search accounts" value={search} onChange={(event) => setSearch(event.target.value)} /></label><button className="button primary" onClick={() => setCreating(true)}><Plus size={17} /> Add account</button></div><div className="user-cards">{data.users.filter(user => `${user.name} ${user.username} ${user.email || ""}`.toLowerCase().includes(search.toLowerCase())).map(account => { const RoleIcon = ROLE_ICONS[account.role]; const supplier = data.suppliers.find((row) => Number(row.id) === Number(account.supplierId)); const emailEnabled = ["planner", "supplier", "ecosystem"].includes(account.role); return <article className="user-card" key={account.id}><span className="user-avatar">{initials(account.name)}</span><div><strong>{account.name}</strong><small>@{account.username}{emailEnabled && account.email ? ` · ${account.email}` : ""}</small>{supplier && <small className="account-eta">{supplier.routeDurationMinutes ? `ETA ${supplier.routeDurationMinutes} min · ${supplier.routeDistanceKm} km${supplier.routeTrafficModel === "TIME_OF_DAY" ? supplier.routeTrafficDelayMinutes ? ` · estimated traffic +${supplier.routeTrafficDelayMinutes} min` : " · estimated traffic" : " · base road time"}` : "ETA route not configured"}</small>}</div><span className="role-chip"><RoleIcon size={14} /> {accountRoleLabel(account)}</span><span className="account-actions">{supplier && <button className="icon-button" title="Configure ETA route" aria-label={`Configure ETA for ${account.name}`} onClick={() => setEtaSupplier(supplier)}><MapPinned size={16} /></button>}<button className="icon-button danger" title={account.id === currentUser.id ? "You cannot delete your current account" : "Delete account"} aria-label={`Delete ${account.name}`} disabled={account.id === currentUser.id} onClick={() => setDeleting(account)}><Trash2 size={16} /></button></span></article>; })}</div></section></>}{creating && <AdminCreateModal onClose={() => setCreating(false)} onSubmit={finishCreate} />}{deleting && <DeleteAccountModal account={deleting} onClose={() => setDeleting(null)} onDelete={async (password) => { await onDeleteUser(deleting, password); setDeleting(null); }} />}{etaSupplier && <SupplierEtaModal supplier={etaSupplier} siteConfigured={Boolean(data.settings.siteAddressConfigured)} onClose={() => setEtaSupplier(null)} onCalculate={async (address, mapReference) => { await onCalculateSupplierEta(etaSupplier, address, mapReference); setEtaSupplier(null); }} />}{receivingSiteOpen && <ReceivingSiteModal onClose={() => setReceivingSiteOpen(false)} onUnlock={onUnlockSiteAddress} onSave={onSaveSiteAddress} />}
  </div>;
}
...
<<<<<<< HEAD
    <main className="main-shell"><header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)}><Menu size={21} /></button><div className="site-identity"><span className="live-dot" /><span>{data.settings.siteName}</span></div><div className="topbar-actions"><LiveClock className="topbar-clock" /><span className="role-badge"><RoleIcon size={15} /> {accountRoleLabel(user)}</span><button className="icon-button" onClick={() => setDark(!dark)} title="Toggle theme">{dark ? <Sun size={18} /> : <Moon size={18} />}</button><NotificationCenter notifications={data.notifications} onOpen={openNotification} onDismiss={(notification) => void markNotificationRead(notification)} /><span className="user-mini profile-only" aria-label={`${user.name} profile`} title={user.name}><span>{initials(user.name)}</span></span></div></header>
      <div className="page-content">{view === "overview" && <OverviewPage data={data} user={user} onOpenShipment={openShipment} />}{view === "monitoring" && <MonitoringPage data={data} theme={dark ? "dark" : "light"} onOpenShipment={openShipment} />}{view === "schedule" && <FlexibleSchedulePage data={data} user={user} onOpenShipment={openShipment} onImportSds={() => setExcelImport(true)} onReviewAlternative={setCompanyDecisionShipment} />}{view === "entries" && <>{user.role === "supplier" && !user.emailVerifiedAt && <section className="verification-reminder"><ShieldCheck size={22} /><div><b>Verify your account email</b><span>This reminder stays here until your email is verified.</span></div><button className="button primary compact" onClick={() => setSelfVerificationOpen(true)}>Verify now</button></section>}<EntriesPage data={data} onOpenShipment={openShipment} /></>}{view === "operations" && <OperationsPage data={data} user={user} onScanStage={scanShipmentStage} onOpenShipment={openShipment} />}{view === "history" && <HistoryPage data={data} user={user} onOpenShipment={openShipment} />}{view === "reports" && <ReportsPage data={data} user={user} token={token} onOpenShipment={openShipment} />}{view === "admin" && <AdminPage data={data} currentUser={user} onAddUser={addUser} onDeleteUser={deleteUser} onUnlockSiteAddress={unlockSiteAddress} onSaveSiteAddress={saveSiteAddress} onCalculateSupplierEta={calculateSupplierEta} />}</div>
=======
    <main className="main-shell"><header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)}><Menu size={21} /></button><div className="site-identity"><span className="live-dot" /><span>{data.settings.siteName}</span></div><div className="topbar-actions"><LiveClock className="topbar-clock" /><NotificationCenter notifications={data.notifications} onOpen={openNotification} onDismiss={(notification) => void markNotificationRead(notification)} /><span className="role-badge"><RoleIcon size={15} /> {accountRoleLabel(user)}</span><button className="icon-button" onClick={() => setDark(!dark)} title="Toggle theme">{dark ? <Sun size={18} /> : <Moon size={18} />}</button><span className="user-mini profile-only" aria-label={`${user.name} profile`} title={user.name}><span>{initials(user.name)}</span></span></div></header>
      <div className="page-content">{view === "sap" && <SapPage token={token} />}{view === "ecosystem" && <EcosystemPage token={token} data={data} user={user} refresh={refresh} />}{view === "overview" && <OverviewPage data={data} user={user} onOpenShipment={openShipment} />}{view === "monitoring" && <MonitoringPage data={data} theme={dark ? "dark" : "light"} onOpenShipment={openShipment} />}{view === "schedule" && <FlexibleSchedulePage data={data} user={user} onOpenShipment={openShipment} onImportSds={() => setExcelImport(true)} onReviewAlternative={setCompanyDecisionShipment} />}{view === "entries" && <>{user.role === "supplier" && !user.emailVerifiedAt && <section className="verification-reminder"><ShieldCheck size={22} /><div><b>Verify your account email</b><span>This reminder stays here until your email is verified.</span></div><button className="button primary compact" onClick={() => setSelfVerificationOpen(true)}>Verify now</button></section>}<EntriesPage data={data} onOpenShipment={openShipment} /></>}{view === "operations" && <OperationsPage data={data} user={user} onScanStage={scanShipmentStage} onOpenShipment={openShipment} />}{view === "history" && <HistoryPage data={data} user={user} onOpenShipment={openShipment} />}{view === "reports" && <ReportsPage data={data} user={user} token={token} onOpenShipment={openShipment} />}{view === "admin" && <AdminPage data={data} currentUser={user} onAddUser={addUser} onDeleteUser={deleteUser} onUnlockSiteAddress={unlockSiteAddress} onSaveSiteAddress={saveSiteAddress} onCalculateSupplierEta={calculateSupplierEta} />}</div>
>>>>>>> 17d9196 (a)
    </main>
