...
function OperationsPage({ data, user, onScanStage, onGateReject, onOpenShipment }: { data: AppData; user: SessionUser; onScanStage: (scanValue: string, stage: ScanStage, options?: { gateDecision?: "ACCEPT" }) => Promise<Shipment>; onGateReject: (shipment: Shipment, reasonCode: string, otherReason?: string) => Promise<Shipment>; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const stageStatuses: Record<ScanStage, ShipmentStatus[]> = { LOOKUP: ["BOOKED", "IN_TRANSIT", "GATE_IN", "UNLOADING", "RECEIVED", "GATE_OUT"], TRIP: ["BOOKED"], GATE: ["BOOKED", "IN_TRANSIT", "UNLOADING", "RECEIVED"], UNLOADING: ["GATE_IN"] };
  ...
  const gateIsExit = stage === "GATE" && Boolean(selected && ["UNLOADING", "RECEIVED", "GATE_OUT"].includes(selected.status));
    ...
    {availableStages.length > 0 && <><section className="operations-grid">
      ...
      <article className="panel verification-panel">
        {!selected ? <EmptyState icon={QrCode} title={`Ready for the ${activeStation.label} scan`} body="Scan the printed booking QR. The entry status updates as soon as the code is accepted." /> : <>
          ...
          <div className="flow-progress">{["Booking", "Trip", "Gate in", "Unload", "Gate out"].map((label, index) => <div className={next && next >= index + 1 ? "done" : ""} key={label}><span>{next && next > index + 1 ? <Check size={13} /> : index + 1}</span><small>{label}</small></div>)}</div>
