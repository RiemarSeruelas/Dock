...
function AdminPage({ data, currentUser, onAddUser, onDeleteUser, onSaveSiteAddress, onCalculateSupplierEta, onSaveSupplierPresets, onSaveAccountEmail, onSendEmailCode, onVerifyEmail }: { data: AppData; currentUser: SessionUser; onAddUser: (form: Record<string, string | number>) => void; onDeleteUser: (account: SessionUser, password: string) => Promise<void> | void; onSaveSiteAddress: (address: string) => Promise<void> | void; onCalculateSupplierEta: (supplier: SupplierAccount, address: string) => Promise<void> | void; onSaveSupplierPresets: (supplier: SupplierAccount, presets: SupplierPreset[]) => Promise<void> | void; onSaveAccountEmail: (account: SessionUser, email: string) => Promise<void>; onSendEmailCode: (account: SessionUser) => Promise<void>; onVerifyEmail: (account: SessionUser, code: string) => Promise<void> }) {
  ...
  return <div className="page-stack"><section className="hero-row"><div><span className="eyebrow">System control</span><h1>Administration</h1></div>{tab === "users" && <button className="button primary" onClick={() => setCreating(true)}><Plus size={17} /> Add account</button>}</section>
    ...
    {tab === "users" && <section className="panel email-admin-card"><span className="settings-icon"><ShieldCheck size={20} /></span><div><strong>Account email verification</strong><small>{data.settings.emailNotifications?.configured ? "Administrator sender is ready · select an account to verify" : "Administrator sender is not configured in the server .env"}</small></div><div className="email-verification-list">{data.users.map((account) => <button key={account.id} disabled={!data.settings.emailNotifications?.configured} onClick={() => setVerificationAccount(account)}><span><b>{account.name}</b><small>{account.email || "No email"}</small></span><em className={account.emailVerifiedAt ? "verified" : "pending"}>{account.emailVerifiedAt ? "Verified" : "Verify"}</em></button>)}</div></section>}
