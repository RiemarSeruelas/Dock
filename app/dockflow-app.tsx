...
export default function DockFlowApp() {
  ...
  const calculateSupplierEta = async (supplier: SupplierAccount, address: string, mapReference: string) => {
    try {
      const result = await apiRequest<{ supplier: SupplierAccount; trafficAware: boolean }>(token, `/api/suppliers/${supplier.id}/route`, "PATCH", { originAddress: address, mapReference });
      await refresh();
      notify(`${supplier.name}: ${result.supplier.routeDurationMinutes} min over ${result.supplier.routeDistanceKm} km${result.trafficAware ? " with live traffic" : " (traffic service unavailable)"}.`);
    } catch (reason) {
