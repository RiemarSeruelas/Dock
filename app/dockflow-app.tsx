...
function AdminCreateModal({ suppliers, onClose, onSubmit }: { suppliers: SupplierAccount[]; onClose: () => void; onSubmit: (form: Record<string, string | number>) => void }) {
  const [form, setForm] = useState<Record<string, string | number>>({ name: "", username: "", email: "", password: "", role: "supplier", supplierId: suppliers[0]?.id || "" });
  const set = (name: string, value: string | number) => setForm({ ...form, [name]: value });
  return <Modal title="Add user account" onClose={onClose}><form className="modal-form" onSubmit={event => { event.preventDefault(); onSubmit(form); }}><div className="form-grid"><label>Account display name<input value={form.name} onChange={event => set("name", event.target.value)} required /></label><label>Username<input value={form.username} onChange={event => set("username", event.target.value.toLowerCase())} required /></label><label>Email<input type="email" value={form.email} onChange={event => set("email", event.target.value.toLowerCase())} required /></label><label>Initial password<input type="password" minLength={8} value={form.password} onChange={event => set("password", event.target.value)} required /></label><label>Account role<select value={form.role} onChange={event => set("role", event.target.value)}>{(Object.keys(ROLE_LABELS) as Role[]).map(role => <option value={role} key={role}>{ROLE_LABELS[role]}</option>)}</select></label>{["supplier", "driver"].includes(String(form.role)) && <label>Supplier company<select value={form.supplierId} onChange={event => set("supplierId", Number(event.target.value))} required><option value="">Choose a supplier</option>{suppliers.map((supplier) => <option value={supplier.id} key={supplier.id}>{supplier.name}</option>)}</select></label>}</div><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary"><Plus size={17} /> Add account</button></div></form></Modal>;
}
...
function EmailSenderModal({ currentEmail, onClose, onSave }: { currentEmail?: string; onClose: () => void; onSave: (email: string, appPassword: string) => Promise<void> | void }) {
  const [email, setEmail] = useState(currentEmail || ""), [appPassword, setAppPassword] = useState(""), [busy, setBusy] = useState(false);
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (busy) return; setBusy(true); try { await onSave(email, appPassword); onClose(); } catch { setBusy(false); } };
  return <Modal title="Notification sender" subtitle="Saved server-side and never returned to the browser after this form is submitted." onClose={onClose}><form className="modal-form" onSubmit={submit}><label>Administrator Gmail<input type="email" value={email} onChange={(event) => setEmail(event.target.value.toLowerCase())} placeholder="dockflow.notifications@gmail.com" required /></label><label>Google App Password<input type="password" autoComplete="new-password" value={appPassword} onChange={(event) => setAppPassword(event.target.value)} minLength={12} required /></label><div className="security-note"><ShieldCheck size={18} /><span><b>Write-only credential</b><small>The App Password is encrypted in trial storage and excluded from bootstrap responses, account data, and logs.</small></span></div><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? <Loader2 className="spin" size={17} /> : <Check size={17} />} Verify & save</button></div></form></Modal>;
}
...
function VerifyAccountEmailModal({ account, onClose, onSaveEmail, onSendCode, onVerify }: { account: SessionUser; onClose: () => void; onSaveEmail: (account: SessionUser, email: string) => Promise<void>; onSendCode: (account: SessionUser) => Promise<void>; onVerify: (account: SessionUser, code: string) => Promise<void> }) {
  ...
  const send = async () => { if (busy) return; setBusy(true); try { if (email !== account.email) await onSaveEmail(account, email); await onSendCode({ ...account, email }); setSent(true); } finally { setBusy(false); } };
  const verify = async (event: React.FormEvent) => { event.preventDefault(); if (busy) return; setBusy(true); try { await onVerify(account, code); onClose(); } catch { setBusy(false); } };
  return <Modal title={`Verify ${account.name}`} subtitle="The code is sent from the configured DockFlow administrator Gmail." onClose={onClose}><form className="modal-form" onSubmit={verify}><label>Account email<input type="email" value={email} onChange={(event) => { setEmail(event.target.value.toLowerCase()); setSent(false); }} required /></label><button type="button" className="button secondary" disabled={busy || !email} onClick={() => void send()}>{busy ? <Loader2 className="spin" size={16} /> : <RefreshCw size={16} />} Send verification code</button>{sent && <label>6-digit code<input inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="xxxxxx" autoFocus required /></label>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={!sent || code.length !== 6 || busy}><Check size={17} /> Verify email</button></div></form></Modal>;
}
...
function AdminPage({ data, currentUser, onAddUser, onDeleteUser, onSaveSiteAddress, onCalculateSupplierEta, onSaveSupplierPresets, onConfigureEmailSender, onSaveAccountEmail, onSendEmailCode, onVerifyEmail }: { data: AppData; currentUser: SessionUser; onAddUser: (form: Record<string, string | number>) => void; onDeleteUser: (account: SessionUser, password: string) => Promise<void> | void; onSaveSiteAddress: (address: string) => Promise<void> | void; onCalculateSupplierEta: (supplier: SupplierAccount, address: string) => Promise<void> | void; onSaveSupplierPresets: (supplier: SupplierAccount, presets: SupplierPreset[]) => Promise<void> | void; onConfigureEmailSender: (email: string, appPassword: string) => Promise<void>; onSaveAccountEmail: (account: SessionUser, email: string) => Promise<void>; onSendEmailCode: (account: SessionUser) => Promise<void>; onVerifyEmail: (account: SessionUser, code: string) => Promise<void> }) {
  ...
  const [verificationAccount, setVerificationAccount] = useState<SessionUser | null>(null);
  ...
  return <div className="page-stack"><section className="hero-row"><div><span className="eyebrow">System control</span><h1>Administration</h1></div>{tab === "users" && <button className="button primary" onClick={() => setCreating(true)}><Plus size={17} /> Add account</button>}</section>
    ...
    {tab === "users" && <section className="panel email-admin-card"><span className="settings-icon"><ShieldCheck size={20} /></span><div><strong>Notification email</strong><small>{data.settings.emailNotifications?.configured ? `${data.settings.emailNotifications.senderEmail} · sender verified` : "Configure an administrator Gmail sender"}</small></div><button className="button secondary compact" onClick={() => setSenderOpen(true)}>{data.settings.emailNotifications?.configured ? "Replace sender" : "Configure sender"}</button><div className="email-verification-list">{data.users.filter((account) => ["admin", "planner", "supplier"].includes(account.role)).map((account) => <button key={account.id} onClick={() => setVerificationAccount(account)}><span><b>{account.name}</b><small>{account.email || "No email"}</small></span><em className={account.emailVerifiedAt ? "verified" : "pending"}>{account.emailVerifiedAt ? "Verified" : "Verify"}</em></button>)}</div></section>}
...
export default function DockFlowApp() {
  ...
  const configureEmailSender = async (email: string, appPassword: string) => persist("/api/admin/email-sender", "PATCH", { email, appPassword }, "Notification sender verified and saved securely.");
  const saveAccountEmail = async (account: SessionUser, email: string) => persist(`/api/users/${account.id}/email`, "PATCH", { email }, `${account.name}'s email saved. Verification is still required.`);
  ...
  const verifyEmail = async (account: SessionUser, code: string) => persist(`/api/users/${account.id}/email/verify`, "POST", { code }, `${account.name}'s email is verified.`);
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <main className="main-shell"><header className="topbar"><button className="menu-button" onClick={() => setMobileNav(true)}><Menu size={21} /></button><div className="site-identity"><span className="live-dot" /><span>{data.settings.siteName}</span></div><div className="topbar-actions"><span className="role-badge"><RoleIcon size={15} /> {ROLE_LABELS[user.role]}</span><button className="icon-button" onClick={() => setDark(!dark)} title="Toggle theme">{dark ? <Sun size={18} /> : <Moon size={18} />}</button><span className="user-mini profile-only" aria-label={`${user.name} profile`} title={user.name}><span>{initials(user.name)}</span></span></div></header>
      <div className="page-content">{view === "overview" && <OverviewPage data={data} user={user} onOpenShipment={setSelectedShipment} />}{view === "monitoring" && <MonitoringPage data={data} onOpenShipment={setSelectedShipment} />}{view === "schedule" && <FlexibleSchedulePage data={data} user={user} onOpenShipment={setSelectedShipment} onSaveAvailability={saveAvailability} onDeleteAvailability={deleteAvailability} onMoveShipment={moveShipment} onImportSds={() => setExcelImport(true)} onSupplierResponse={respondToSds} />}{view === "entries" && <EntriesPage data={data} onOpenShipment={setSelectedShipment} />}{view === "operations" && <OperationsPage data={data} user={user} onScanStage={scanShipmentStage} onOpenShipment={setSelectedShipment} />}{view === "history" && <HistoryPage data={data} user={user} onOpenShipment={setSelectedShipment} />}{view === "reports" && <ReportsPage data={data} user={user} token={token} onOpenShipment={setSelectedShipment} />}{view === "admin" && <AdminPage data={data} currentUser={user} onAddUser={addUser} onDeleteUser={deleteUser} onSaveSiteAddress={saveSiteAddress} onCalculateSupplierEta={calculateSupplierEta} onSaveSupplierPresets={saveSupplierPresets} onConfigureEmailSender={configureEmailSender} onSaveAccountEmail={saveAccountEmail} onSendEmailCode={sendEmailCode} onVerifyEmail={verifyEmail} />}</div>
    </main>
