...
function AdminCreateModal({ currentUser, onClose, onSubmit }: { currentUser: SessionUser; onClose: () => void; onSubmit: (form: Record<string, string | number>) => Promise<void> | void }) {
  const [form, setForm] = useState<Record<string, string | number>>({ name: "", username: "", email: "", password: "", role: "supplier", workArea: currentUser.workArea || "DRESSINGS", securityScope: "COMPANY" });
  ...
  return <Modal title="Add user account" onClose={onClose}><form className="modal-form" onSubmit={async event => { event.preventDefault();if(busy)return;setBusy(true);setError("");try{await onSubmit({...form,email:form.email});}catch(error){setError(error instanceof Error ? error.message : "Unable to add account");}finally{setBusy(false);} }}><div className="form-grid"><label>Account display name<input value={form.name} onChange={event => set("name", event.target.value)} required /></label><label>Username<input value={form.username} onChange={event => set("username", event.target.value.toLowerCase())} required /></label>{needsEmail && <label>Email<input type="email" value={form.email} onChange={event => set("email", event.target.value.toLowerCase())} required /></label>}<label>Initial password<span className="activation-password admin-create-password" onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))setPasswordVisible(false);}}><input type={passwordVisible?"text":"password"} minLength={8} value={form.password} onChange={event => set("password", event.target.value)} required /><button type="button" onClick={()=>setPasswordVisible(current=>!current)} aria-label={passwordVisible?"Hide password":"Show password"} title={passwordVisible?"Hide password":"Show password"}>{passwordVisible?<EyeOff size={17}/>:<Eye size={17}/>}</button></span></label><label>Account role<select value={form.role} onChange={event => { const nextRole = event.target.value; setForm(current => ({ ...current, role: nextRole, workArea: nextRole === "supplier" ? "" : currentUser.workArea || (nextRole === "admin" ? "" : "DRESSINGS"), securityScope: nextRole === "security" ? "COMPANY" : current.securityScope })); }}>{roleOptions.map(({ role: optionRole, label }) => <option value={optionRole} key={optionRole}>{label}</option>)}</select></label>{role === "security" && <label>Security access<select value={form.securityScope} onChange={event => set("securityScope", event.target.value)}><option value="COMPANY">Company deliveries</option><option value="ECOSYSTEM">Ecosystem deliveries</option></select></label>}{areaChoice && <label>Access area{currentUser.workArea ? <span className="locked-access-area"><ShieldCheck size={17} /><span><b>{currentUser.workArea === "DRESSINGS" ? "Dressings" : "Savoury"}</b><small>Locked to your administrator area</small></span></span> : <select value={form.workArea} onChange={event => set("workArea", event.target.value as WorkArea)} required={areaRequired}>{role === "admin" && <option value="">Both</option>}<option value="DRESSINGS">Dressings</option><option value="SAVOURY">Savoury</option></select>}</label>}</div>{error&&<p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}><Plus size={17} /> Add account</button></div></form></Modal>;
}
...
function AdminPage({ data, token, currentUser, onAddUser, onDeleteUser, onUnlockSiteAddress, onSaveSiteAddress, onCalculateSupplierEta }: { data: AppData; token: string; currentUser: SessionUser; onAddUser: (form: Record<string, string | number>) => Promise<void> | void; onDeleteUser: (account: SessionUser, password: string) => Promise<void> | void; onUnlockSiteAddress: (password: string) => Promise<{ siteAddress: string; configured: boolean }>; onSaveSiteAddress: (address: string, mapReference: string, password: string) => Promise<void>; onCalculateSupplierEta: (supplier: SupplierAccount, address: string, mapReference: string) => Promise<void> | void }) {
  ...
  const accountGroups = [
    ...
    { key: "BOTH", title: "Shared services", helper: "Global administrators, Company/Ecosystem Security, and Ecosystem", accounts: matchingAccounts.filter(account => account.role !== "supplier" && (!account.workArea || account.workArea === "ECOSYSTEM")) },
    { key: "DRESSINGS", title: "Dressings", helper: "Company accounts locked to Dressings deliveries", accounts: matchingAccounts.filter(account => account.role !== "supplier" && account.workArea === "DRESSINGS") },
    { key: "SAVOURY", title: "Savoury", helper: "Company accounts locked to Savoury deliveries", accounts: matchingAccounts.filter(account => account.role !== "supplier" && account.workArea === "SAVOURY") },
...
export default function DockFlowApp() {
  ...
  const canSwitchArea = (user.role === "security" && !ecosystemSecurity) || (user.role === "admin" && !user.workArea);
