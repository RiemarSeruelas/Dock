...
type ImportPreview = {
  ...
  summary: { totalRows: number; readyRows: number; skippedRows: number; deliveryGroups: number; poMatchedRows: number; warningRows: number; missingSupplierAccounts: number };
...
export function ExcelImportModal({ token, onClose, onImported }: { token: string; onClose: () => void; onImported: (result: ImportResult) => void }) {
  ...
  const commit = async () => {
    ...
    try {
      const response = await authenticatedFetch("/api/imports/excel/commit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ previewToken: preview.previewToken }) }, token);
