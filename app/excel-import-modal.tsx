...
export type ImportResult = { deliveryCount: number; importedRows: number; skippedRows: number; notification?: { status: string; sent: number; failed: number } };
...
export function ExcelImportModal({ token, onClose, onImported }: { token: string; onClose: () => void; onImported: (result: ImportResult) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal modal-wide excel-modal" role="dialog" aria-modal="true" aria-label="Import SDS workbook"><div className="modal-head"><div><h2>Import SDS workbook</h2><p>Every nonblank material row is accepted. Each supplier receives one proposal to confirm its time and truck load.</p></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div>
    <div className="excel-body">
      {!preview && <><label className="excel-drop"><span className="excel-file-icon"><FileSpreadsheet size={28} /></span><span><b>{file ? file.name : "Choose a spreadsheet"}</b><small>Excel, OpenDocument, CSV, or TSV · maximum 25 MB</small></span><input type="file" accept=".xlsx,.xlsm,.xls,.xlsb,.xltx,.xltm,.ods,.csv,.tsv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv" onChange={(event) => { setFile(event.target.files?.[0] || null); setError(""); }} /></label><div className="import-rule-grid"><article><b>Common spreadsheet formats</b><p>Modern and legacy Excel workbooks, templates, OpenDocument sheets, CSV, and TSV files are accepted.</p></article><article><b>Missing data</b><p>The trial uses clear placeholders that can be reviewed later instead of dropping the row.</p></article><article><b>No duplicate blocking</b><p>Repeated-looking rows, different materials, and different dates are all imported as supplied.</p></article></div></>}
