...
export function FlexibleSchedulePage({ data, user, area, canSwitchArea, ecosystemMode = false, onAreaChange, onOpenShipment, onImportSds, onReviewAlternative, onUpdateSchedule, onDownloadSds }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; ecosystemMode?: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onReviewAlternative: (shipment: Shipment) => void; onUpdateSchedule: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void>; onDownloadSds: (area: ReceivingArea) => Promise<void> }) {
  ...
  const areaData = useMemo(() => ({ ...data, shipments: ecosystemMode ? data.shipments : data.shipments.filter(shipment => shipmentArea(shipment) === area) }), [data, area, ecosystemMode]);
  ...
  return <div className="page-stack">
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      <div className="panel-head schedule-board-head"><div><span className="eyebrow">{period}</span><h2>Booked delivery schedule</h2></div><div className="schedule-view-controls schedule-command-bar"><span className="approved-delivery-count"><i/><b>{bookingCount}</b> Approved Deliveries</span>{canReviewCompany&&<SdsWorkflowPanel data={areaData} onOpenShipment={onOpenShipment} onReviewAlternative={onReviewAlternative}/>}<div className="view-toggle"><button className={mode === "day" ? "active" : ""} onClick={() => setMode("day")}>Day</button><button className={mode === "week" ? "active" : ""} onClick={() => setMode("week")}>Week</button></div><div className="date-stepper"><button type="button" onClick={() => move(-1)} aria-label={`Previous ${mode}`}><ArrowLeft size={17} /></button><CustomDatePicker value={date} onChange={setDate} label="Schedule date" /><button type="button" onClick={() => move(1)} aria-label={`Next ${mode}`}><ArrowRight size={17} /></button></div></div></div>
      ...
      <div className="monitor-grid">{rows.map((shipment) => {
        ...
        return <button className={`monitor-delivery-card monitor-tone-${STATUS_META[shipment.status].color} ${ecosystemScope ? `ecosystem-${ecosystemScope}` : ""}`} key={shipment.id} onClick={() => void openMonitoringEntry(shipment)}>
          <span className="monitor-card-head">
            <span>{ecosystemScope && <em className={`monitor-direction-chip ${ecosystemScope}`}>{ecosystemScope}</em>}<small>{formatDate(shipment.scheduledDate)}</small><b>{shipment.scheduledTime}</b></span>
