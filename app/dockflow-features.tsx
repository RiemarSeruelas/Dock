...
const STATUS_META: Record<ShipmentStatus, { label: string; color: string }> = {
  PROPOSED: { label: "Supplier approval", color: "purple" }, BOOKED: { label: "Booked", color: "slate" }, IN_TRANSIT: { label: "Awaiting gate in", color: "blue" }, GATE_IN: { label: "Gate in", color: "amber" },
...
const journeySteps = ["Booking", "Gate in", "Unload", "Received", "Gate out"];
const journeyPosition: Record<ShipmentStatus, number> = { PROPOSED: 0, BOOKED: 1, IN_TRANSIT: 1, GATE_IN: 2, UNLOADING: 3, RECEIVED: 4, GATE_OUT: 5, REJECTED: 0 };
...
export function MonitoringPage({ data, theme, onOpenShipment }: { data: AppData; theme: "light" | "dark"; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  return <div className={`page-stack monitoring-page tv-${theme} ${fullscreen ? "tv-mode" : ""}`}>
    ...
    {!fullscreen && <section className="monitor-status-strip">{statusOrder.filter((item) => !["PROPOSED", "IN_TRANSIT", "GATE_OUT", "REJECTED"].includes(item)).map((item) => <button key={item} className={status === item ? "active" : ""} onClick={() => setStatus(status === item ? "ALL" : item)}><StatusPill status={item} /><b>{active.filter((shipment) => shipment.status === item).length}</b></button>)}</section>}
