...
import { AlertTriangle, ArrowLeft, ArrowRight, CalendarDays, Check, Clock3, Download, FileSpreadsheet, History, Loader2, Maximize2, Minimize2, Plus, Search, Truck, X } from "lucide-react";
...
export function FlexibleSchedulePage({ data, user, area, canSwitchArea, ecosystemMode = false, unfilteredMode = false, onAreaChange, onOpenShipment, onImportSds, onRequestDelivery, onReviewAlternative, onUpdateSchedule, onDownloadSds }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; ecosystemMode?: boolean; unfilteredMode?: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onRequestDelivery: () => void; onReviewAlternative: (shipment: Shipment) => void; onUpdateSchedule: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void>; onDownloadSds: (area: ReceivingArea) => Promise<void> }) {
  ...
  return <div className="page-stack">
    ...
    <section className="hero-row"><div><span className="eyebrow">Scheduling center</span><h1>Delivery schedule</h1></div><div className="hero-actions">{["admin", "planner", "supplier"].includes(user.role) && <button className="button primary" onClick={onRequestDelivery}><Plus size={17}/> Request</button>}{ecosystemMode ? <span className="area-scope-chip ecosystem-scope-chip">Receiving &amp; sending</span> : unfilteredMode ? <span className="area-scope-chip ecosystem-scope-chip receiving">Ecosystem receiving</span> : canSwitchArea ? <AreaToggle value={area} onChange={onAreaChange}/> : <span className="area-scope-chip">{area === "DRESSINGS" ? "Dressings" : "Savoury"}</span>}</div></section>
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      ...
      {canImport && <div className="schedule-data-footer"><span><b>Schedule data</b><small>{canManage ? "Select any delivery ticket to change its schedule and notify the supplier." : "Import or download the current schedule."}</small></span><div><button className="button secondary" onClick={() => void onDownloadSds(area)}><Download size={17}/> Updated SDS</button><button className="button primary import-sds-button" onClick={onImportSds}><FileSpreadsheet size={17}/> Import SDS</button></div></div>}
    </section>
  </div>;
}
