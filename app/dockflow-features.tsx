...
export type SupplierResponsePayload = {
  ...
  trucks?: { truckPlate: string; driverName: string; driverPhone: string; helper1Name?: string; helper2Name?: string; poNumber?: string; drNumber?: string; itemIds: number[] }[];
};
...
export function CompanyDecisionModal({ shipment, onClose, onSubmit }: { shipment: Shipment; onClose: () => void; onSubmit: (shipment: Shipment, payload: CompanyDecisionPayload) => Promise<void> | void }) {
  ...
  const dialog = <div className="modal-backdrop sds-fullscreen-backdrop" role="presentation"><section className="modal company-decision-modal" role="dialog" aria-modal="true" aria-label="Review supplier reschedule request"><div className="modal-head"><div><h2>Review reschedule request</h2><p>{shipment.supplier} · {shipment.shipmentNumber}</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close"><X size={20} /></button></div><form onSubmit={submit} className="company-decision-form">
    ...
    <div className="company-decision-choices"><button type="button" className={decision === "APPROVE" ? "active approve" : ""} onClick={() => setDecision("APPROVE")}><Check size={18} /><span><b>Approve reschedule</b><small>The approved schedule returns to the supplier for delivery details.</small></span></button><button type="button" className={decision === "REJECT" ? "active reject" : ""} onClick={() => setDecision("REJECT")}><X size={18} /><span><b>Reject reschedule</b><small>The reason remains visible in the supplier app.</small></span></button></div>
...
export function FlexibleSchedulePage({ data, user, onOpenShipment, onImportSds, onReviewAlternative }: { data: AppData; user: SessionUser; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onReviewAlternative: (shipment: Shipment) => void }) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      <div className="panel-head schedule-board-head"><div><span className="eyebrow">{period}</span><h2>Booked delivery schedule</h2></div><div className="schedule-view-controls"><div className="view-toggle"><button className={mode === "day" ? "active" : ""} onClick={() => setMode("day")}>Day</button><button className={mode === "week" ? "active" : ""} onClick={() => setMode("week")}>Week</button></div><div className="date-stepper"><button type="button" onClick={() => move(-1)} aria-label={`Previous ${mode}`}><ArrowLeft size={17} /></button><CustomDatePicker value={date} onChange={setDate} label="Schedule date" /><button type="button" onClick={() => move(1)} aria-label={`Next ${mode}`}><ArrowRight size={17} /></button></div><span className="policy-chip"><Clock3 size={14} /> Manila time · GMT+8</span></div></div>
