...
function SdsWorkflowPanel({ data, user, onImportSds, onSupplierResponse }: { data: AppData; user: SessionUser; onImportSds: () => void; onSupplierResponse: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void }) {
  ...
  const rows = user.role === "supplier" ? supplierPending : unlinked;
  return <section className="panel sds-workflow-panel"><div className="panel-head"><div><span className="eyebrow">SDS workflow</span><h2>{user.role === "supplier" ? "Confirm proposed deliveries" : "SDS imports"}</h2></div>{planning && <button className="button primary" onClick={onImportSds}><FileSpreadsheet size={17} /> Import SDS</button>}</div>
    <div className="sds-workflow-list">{rows.map((shipment) => { const group = data.shipments.filter((row) => Number(row.sdsProposalId || row.id) === Number(shipment.sdsProposalId || shipment.id)); const site = shipment.items.find((item) => item.deliverySite)?.deliverySite || "Site not supplied"; const week = shipment.items.find((item) => item.deliveryWeek)?.deliveryWeek; const remaining = shipment.items.filter((item) => !item.supplierApprovedAt).length; return <article className={shipment.supplierAccountLinked === false ? "account-unlinked" : ""} key={shipment.id}><span className="truck-tile"><Truck size={19} /></span><div className="review-main"><strong>{shipment.supplier}</strong><small>{site}{week ? ` · Week ${week}` : ""}</small><span>{group.flatMap((row) => row.items).map((item) => `${item.materialCode}: ${item.quantity.toLocaleString()} ${item.uom}`).join(" · ")}</span>{shipment.supplierAccountLinked === false && <span className="account-missing-label"><AlertTriangle size={13} /> No supplier account</span>}</div><div className="review-schedule"><strong>{formatDate(shipment.scheduledDate, true)}</strong><small>{shipment.scheduledTime}</small>{user.role === "supplier" && <span>{remaining} material code{remaining === 1 ? "" : "s"} remaining</span>}</div>{user.role === "supplier" ? <button className="button primary compact" onClick={() => setResponding(shipment)}>{shipment.confirmedTruckLoads?.length ? "Confirm next truck" : "Review & confirm"}</button> : <span className="approval-chip pending">Account required</span>}</article>; })}</div>
    {!rows.length && <div className="feature-empty management-empty"><ShieldCheck size={24} /><strong>{user.role === "supplier" ? "No SDS proposal waiting" : "No account issues"}</strong><span>{user.role === "supplier" ? "A new SDS upload assigned to your account will appear here." : "Import a workbook to create or compare supplier proposals."}</span></div>}
...
export function FlexibleSchedulePage({ data, user, onOpenShipment, onSaveAvailability, onDeleteAvailability, onMoveShipment, onImportSds, onSupplierResponse }: { data: AppData; user: SessionUser; onOpenShipment: (shipment: Shipment) => void; onSaveAvailability: (slot: AvailabilityInput) => Promise<void> | void; onDeleteAvailability: (id: number) => Promise<void> | void; onMoveShipment: (shipment: Shipment, date: string, startTime: string, endTime: string) => Promise<void> | void; onImportSds: () => void; onSupplierResponse: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void }) {
  ...
  return <div className="page-stack">
    ...
    <SdsWorkflowPanel data={data} user={user} onImportSds={onImportSds} onSupplierResponse={onSupplierResponse} />
