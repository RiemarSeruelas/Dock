...
export function CompanyDecisionModal({ shipment, onClose, onSubmit }: { shipment: Shipment; onClose: () => void; onSubmit: (shipment: Shipment, payload: CompanyDecisionPayload) => Promise<void> | void }) {
  ...
  const dialog = <div className="modal-backdrop sds-fullscreen-backdrop" role="presentation"><section className="modal company-decision-modal" role="dialog" aria-modal="true" aria-label="Review supplier schedule proposal"><div className="modal-head"><div><h2>Review proposed schedule</h2><p>{shipment.supplier} · {shipment.shipmentNumber}</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close"><X size={20} /></button></div><form onSubmit={submit} className="company-decision-form">
    ...
    <div className="company-decision-choices"><button type="button" className={decision === "APPROVE" ? "active approve" : ""} onClick={() => setDecision("APPROVE")}><Check size={18} /><span><b>Approve proposed time</b><small>The supplier must then confirm the truck and driver.</small></span></button><button type="button" className={decision === "REJECT" ? "active reject" : ""} onClick={() => setDecision("REJECT")}><X size={18} /><span><b>Reject proposed time</b><small>The supplier will receive the company reason.</small></span></button></div>
    ...
    <div className="company-decision-actions">{attempted && !valid ? <span className="form-error"><AlertTriangle size={16} />A rejection reason is required.</span> : <span>The supplier receives an in-app and email notification.</span>}<div><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className={`button ${decision === "APPROVE" ? "primary" : "danger"}`} disabled={busy}>{busy ? <Loader2 className="spin" size={17} /> : decision === "APPROVE" ? <Check size={17} /> : <X size={17} />}{decision === "APPROVE" ? "Approve proposal" : "Reject proposal"}</button></div></div>
  </form></section></div>;
  return createPortal(dialog, document.body);
}
...
function SdsWorkflowPanel({ data, showImport, onImportSds, onOpenShipment, onReviewAlternative }: { data: AppData; showImport: boolean; onImportSds: () => void; onOpenShipment: (shipment: Shipment) => void; onReviewAlternative: (shipment: Shipment) => void }) {
  ...
  return <section className="sds-command-grid">{showImport && <article className="panel sds-import-panel"><div className="panel-head"><div><span className="eyebrow">SDS workflow</span><h2>Import & account checks</h2></div><button className="button primary" onClick={onImportSds}><FileSpreadsheet size={17} /> Import SDS</button></div>{unlinked.length ? <div className="sds-account-issues">{unlinked.map((shipment) => <button key={shipment.id} onClick={() => onOpenShipment(shipment)}><AlertTriangle size={16} /><span><b>{shipment.supplier}</b><small>No supplier account · {formatDate(shipment.scheduledDate, true)} {shipment.scheduledTime}</small></span></button>)}</div> : <div className="feature-empty management-empty"><ShieldCheck size={24} /><strong>No account issues</strong><span>Every imported supplier is linked to an account.</span></div>}</article>}
    <article className="panel company-review-panel"><div className="panel-head"><div><span className="eyebrow">Company decision</span><h2>Alternative schedules to review</h2></div><span className="count-chip warning">{companyReview.length} pending</span></div><div className="company-review-scroll">{companyReview.map((shipment) => <button type="button" className="company-review-row" key={shipment.id} onClick={() => onReviewAlternative(shipment)}><span className="response-status-icon"><CalendarDays size={16} /></span><span className="response-copy"><b>{shipment.supplier}</b><small>{shipment.shipmentNumber}</small><em>{shipment.supplierResponseReason || "No reason recorded"}</em></span><span className="proposal-compare"><small>{shipment.scheduledDate} · {shipment.scheduledTime}</small><ArrowRight size={14} /><b>{shipment.alternativeDate || "—"} · {shipment.alternativeTime || "—"}</b></span><span className="response-open">Review <ArrowRight size={14} /></span></button>)}{!companyReview.length && <span className="outcome-empty"><Check size={18} />No alternative schedules need a decision</span>}</div></article>
...
export function FlexibleSchedulePage({ data, user, onOpenShipment, onImportSds, onReviewAlternative }: { data: AppData; user: SessionUser; onOpenShipment: (shipment: Shipment) => void; onSaveAvailability: (slot: AvailabilityInput) => Promise<void> | void; onDeleteAvailability: (id: number) => Promise<void> | void; onMoveShipment: (shipment: Shipment, date: string, startTime: string, endTime: string) => Promise<void> | void; onImportSds: () => void; onReviewAlternative: (shipment: Shipment) => void }) {
  ...
  return <div className="page-stack">
    ...
    {canReviewCompany && <SdsWorkflowPanel data={data} showImport={canImport} onImportSds={onImportSds} onOpenShipment={onOpenShipment} onReviewAlternative={onReviewAlternative} />}
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      ...
      <div className="schedule-timeline-viewport"><ScheduleTimeline shipments={data.shipments} availableSlots={data.settings.availableSlots} anchorDate={date} mode={mode} showPending={["admin", "planner", "production", "supplier"].includes(user.role)} onOpenShipment={onOpenShipment} /></div>
    </section>
  </div>;
}
