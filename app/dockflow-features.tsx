...
function SupplierSdsModal({ shipment, onClose, onSubmit }: { shipment: Shipment; onClose: () => void; onSubmit: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void }) {
  ...
  const valid = Boolean(validAlternative && truck.truckPlate.trim() && truck.driverName.trim() && validPhone && selectedItems.length > 0);
  const validationMessage = !validAlternative ? "Complete the alternative date, time, and reason." : !truck.truckPlate.trim() ? "Enter the truck plate." : !truck.driverName.trim() ? "Enter the driver name." : !validPhone ? `Enter ${country.min === country.max ? country.max : `${country.min}–${country.max}`} digits after ${country.dial}.` : !selectedItems.length ? "Select at least one material code for this delivery." : "";
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      await onSubmit(shipment, {
        ...
        trucks: [{ ...truck, driverName: truck.driverName.trim(), driverPhone: `${country.dial}${phoneNumber}`, truckPlate: truck.truckPlate.trim().toUpperCase(), itemIds: selectedItems }],
      });
      onClose();
...
export function HistoryPage({ data, onOpenShipment }: { data: AppData; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const rows = source.filter((shipment) => {
    ...
    return searchable.includes(query.trim().toLowerCase()) && (supplier === "ALL" || shipment.supplier === supplier) && (material === "ALL" || shipment.items.some((item) => item.materialCode === material)) && (driver === "ALL" || shipment.driverName === driver) && (outcome === "ALL" || shipment.status === outcome) && (!fromDate || shipment.scheduledDate >= fromDate) && (!toDate || shipment.scheduledDate <= toDate) && (!fromTime || shipment.scheduledTime >= fromTime) && (!toTime || shipment.scheduledTime <= toTime);
  });
