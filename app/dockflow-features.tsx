...
function ScheduleTimeline({ shipments, anchorDate, mode, showPending, canManage, onOpenShipment }: { shipments: Shipment[]; anchorDate: string; mode: "day" | "week"; showPending: boolean; canManage: boolean; onOpenShipment: (shipment: Shipment) => void }) {
...
export function FlexibleSchedulePage({ data, user, area, canSwitchArea, onAreaChange, onOpenShipment, onImportSds, onReviewAlternative, onUpdateSchedule, onDownloadSds }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onReviewAlternative: (shipment: Shipment) => void; onUpdateSchedule: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void>; onDownloadSds: (area: ReceivingArea) => Promise<void> }) {
  ...
  const areaData = useMemo(() => ({ ...data, shipments: data.shipments.filter(shipment => shipmentArea(shipment) === area) }), [data, area]);
  ...
  const pendingSupplierBookings = ["supplier", "ecosystem"].includes(user.role) ? areaData.shipments.filter((shipment) => shipment.bookingStatus === "PENDING_SUPPLIER").sort((a, b) => `${a.scheduledDate}${a.scheduledTime}`.localeCompare(`${b.scheduledDate}${b.scheduledTime}`)) : [];
  return <div className="page-stack">
    {editing && <ScheduleEditModal shipment={editing} onClose={() => setEditing(null)} onSubmit={onUpdateSchedule}/>} 
    <section className="hero-row"><div><span className="eyebrow">Scheduling center</span><h1>Delivery schedule</h1></div><div className="hero-actions">{canSwitchArea ? <AreaToggle value={area} onChange={onAreaChange}/> : <span className="area-scope-chip">{area === "DRESSINGS" ? "Dressings" : "Savoury"}</span>}</div></section>
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      ...
      <div className="supplier-legend">{Array.from(new Map(areaData.shipments.filter(row=>visibleDates.includes(row.scheduledDate)).map(row=>[row.supplierId||row.supplier,row])).values()).map(row=><span key={row.supplierId||row.supplier}><i style={{background:`hsl(${colorFor(row)} 65% 48%)`}}/>{row.supplier}</span>)}</div>
      {mode === "day" && <div className="schedule-day-details">{areaData.shipments.filter(row=>row.scheduledDate===date && row.bookingStatus!=="REJECTED").sort((a,b)=>a.scheduledTime.localeCompare(b.scheduledTime)).map(row=><button key={row.id} style={{borderLeftColor:`hsl(${colorFor(row)} 65% 48%)`}} onClick={()=>openSchedule(row)}><b>{row.scheduledTime}</b><span><strong>{row.supplier}</strong><small>{row.items.map(item=>`${item.materialCode} · ${item.quantity} ${item.uom}`).join(" / ")}</small></span><span>{row.truckPlate||"Waiting for Confirmation"}<small>{row.shipmentNumber}</small></span>{canManage && <em className="schedule-list-edit">Change</em>}</button>)}</div>}
      <div className="schedule-timeline-viewport"><ScheduleTimeline shipments={areaData.shipments} anchorDate={date} mode={mode} showPending={["admin", "planner", "supplier", "ecosystem"].includes(user.role)} canManage={canManage} onOpenShipment={openSchedule} /></div>
...
export function MonitoringPage({ data, theme, area, canSwitchArea, onAreaChange, onOpenShipment }: { data: AppData; theme: "light" | "dark"; area: ReceivingArea; canSwitchArea: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const active = useMemo(() => data.shipments.filter((shipment) => shipmentArea(shipment) === area && shipment.bookingStatus === "APPROVED" && !["GATE_OUT", "REJECTED"].includes(shipment.status)), [data.shipments, area]);
