...
import { AlertTriangle, ArrowLeft, ArrowRight, CalendarDays, Check, Clock3, Download, FileSpreadsheet, History, Loader2, Maximize2, Minimize2, Search, Truck, X } from "lucide-react";
  ...
  return <div className={`schedule-timeline mode-${mode} unified-schedule`}>
    ...
    {days.map((day) => <div className="schedule-day-lane" key={day}>
      ...
      {layoutDayBookings(shipments.filter((shipment) => shipment.bookingStatus === "APPROVED" || (showPending && ["PENDING_SUPPLIER", "PENDING_COMPANY"].includes(shipment.bookingStatus || ""))), day).map(({ shipment, lane, lanes, start, end }) => {
        ...
        return <button type="button" className={`schedule-entry ${pending ? "proposal" : "approved"} ${ecosystemSupplierId ? sending ? "ecosystem-sending" : "ecosystem-receiving" : ""}`} style={{ "--event-hue": scheduleHue(shipment, ecosystemSupplierId), position: "absolute", top: `${Math.max(dayStart, start) / dayEnd * 100}%`, height: `${Math.max(2.1, (Math.min(dayEnd, end) - Math.max(dayStart, start)) / dayEnd * 100)}%`, left: `calc(${lane / lanes * 100}% + 4px)`, width: `calc(${100 / lanes}% - 8px)` } as CSSProperties} key={shipment.id} title={canManage ? "Click to change this delivery schedule" : "Click to view this delivery"} onClick={() => onOpenShipment(shipment)}>{canManage && <em className="schedule-edit-badge">Edit</em>}<b>{shipment.scheduledTime}</b><span>{pendingCompany ? "Company review" : pendingSupplier ? ecosystemSupplierId && !sending ? "Waiting for supplier" : "Waiting for confirmation" : shipment.truckPlate}</span><small>{ecosystemSupplierId ? `${sending ? "Sending" : "Receiving"} · ` : ""}{shipment.supplier}{pending ? " · not booked yet" : " · booked"}</small></button>;
      })}
    </div>)}
  </div>;
}
...
export function FlexibleSchedulePage({ data, user, area, canSwitchArea, ecosystemMode = false, unfilteredMode = false, onAreaChange, onOpenShipment, onImportSds, onReviewAlternative, onUpdateSchedule, onDownloadSds }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; ecosystemMode?: boolean; unfilteredMode?: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onReviewAlternative: (shipment: Shipment) => void; onUpdateSchedule: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void>; onDownloadSds: (area: ReceivingArea) => Promise<void> }) {
  ...
  const openSchedule = (shipment: Shipment) => canManage && !["GATE_IN", "UNLOADING", "RECEIVED", "GATE_OUT", "REJECTED"].includes(shipment.status) ? setEditing(shipment) : onOpenShipment(shipment);
  ...
  return <div ref={fullscreenRef} className={`page-stack monitoring-page tv-${theme} ${fullscreen ? "tv-mode" : ""}`}>
    ...
    <section className="panel monitoring-panel">
      ...
      <div className="monitor-grid">{rows.map((shipment) => {
        ...
        return <button className={`monitor-delivery-card monitor-tone-${STATUS_META[shipment.status].color} ${ecosystemScope ? `ecosystem-${ecosystemScope}` : ""}`} key={shipment.id} onClick={() => void openMonitoringEntry(shipment)}>
