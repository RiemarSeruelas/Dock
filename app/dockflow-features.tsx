...
import { AlertTriangle, ArrowLeft, ArrowRight, CalendarDays, Check, CheckCircle2, Clock3, Download, FileSpreadsheet, History, Loader2, MapPin, Maximize2, Minimize2, Plus, Search, Truck, X } from "lucide-react";
...
type ReceivingArea = "DRESSINGS" | "SAVOURY";
...
const STATUS_META: Record<ShipmentStatus, { label: string; color: string }> = {
  PROPOSED: { label: "Supplier approval", color: "purple" }, BOOKED: { label: "Booked", color: "slate" }, IN_TRANSIT: { label: "In transit", color: "blue" }, GATE_IN: { label: "Gate in", color: "amber" }, SAP_CLEARANCE: { label: "SAP Clearance", color: "violet" },
...
const statusOrder: ShipmentStatus[] = ["PROPOSED", "BOOKED", "IN_TRANSIT", "GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED", "GATE_OUT", "REJECTED"];
const journeySteps = ["Booking", "Trip", "Gate in", "SAP Clearance", "Unload", "Received", "Gate out"];
const journeyPosition: Record<ShipmentStatus, number> = { PROPOSED: 0, BOOKED: 1, IN_TRANSIT: 2, GATE_IN: 3, SAP_CLEARANCE: 4, UNLOADING: 5, RECEIVED: 6, GATE_OUT: 7, REJECTED: 0 };
const processRank: Record<ShipmentStatus, number> = { PROPOSED: 0, BOOKED: 1, IN_TRANSIT: 2, GATE_IN: 3, SAP_CLEARANCE: 4, UNLOADING: 5, RECEIVED: 6, GATE_OUT: 7, REJECTED: 0 };
...
const durationLabel = (minutes: number) => `${Math.floor(Math.abs(minutes) / 60) ? `${Math.floor(Math.abs(minutes) / 60)}h ` : ""}${Math.abs(minutes) % 60}m`;
...
const ECOSYSTEM_SENDING_HUE = 171;
const isEcosystemSending = (shipment: Shipment, ecosystemSupplierId?: number | null) => Boolean(ecosystemSupplierId) && Number(shipment.supplierId) === Number(ecosystemSupplierId);
const scheduleHue = (shipment: Shipment, ecosystemSupplierId?: number | null) => isEcosystemSending(shipment, ecosystemSupplierId) ? ECOSYSTEM_SENDING_HUE : colorFor(shipment);
...
function ScheduleTimeline({ shipments, anchorDate, mode, showPending, canManage, ecosystemSupplierId, onOpenShipment }: { shipments: Shipment[]; anchorDate: string; mode: "day" | "week"; showPending: boolean; canManage: boolean; ecosystemSupplierId?: number | null; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  return <div className={`schedule-timeline mode-${mode} unified-schedule`}>
    ...
    {days.map((day) => <div className="schedule-day-lane" key={day}>
      ...
      {layoutDayBookings(shipments.filter((shipment) => shipment.bookingStatus === "APPROVED" || (showPending && ["PENDING_SUPPLIER", "PENDING_COMPANY"].includes(shipment.bookingStatus || ""))), day).map(({ shipment, lane, lanes, start, end }) => {
        ...
        const sending = isEcosystemSending(shipment, ecosystemSupplierId);
        const viewerName = canManage || ecosystemSupplierId ? shipment.supplier : `Cavite Foods ${shipmentArea(shipment) === "DRESSINGS" ? "Dressings" : "Savoury"}`;
        return <button type="button" className={`schedule-entry ${pending ? "proposal" : "approved"} ${ecosystemSupplierId ? sending ? "ecosystem-sending" : "ecosystem-receiving" : ""}`} style={{ "--event-hue": scheduleHue(shipment, ecosystemSupplierId), position: "absolute", top: `${Math.max(dayStart, start) / dayEnd * 100}%`, height: `${Math.max(2.1, (Math.min(dayEnd, end) - Math.max(dayStart, start)) / dayEnd * 100)}%`, left: `calc(${lane / lanes * 100}% + 4px)`, width: `calc(${100 / lanes}% - 8px)` } as CSSProperties} key={shipment.id} title={canManage ? "Click to change this delivery schedule" : "Click to view this delivery"} onClick={() => onOpenShipment(shipment)}>{canManage && <em className="schedule-edit-badge">Edit</em>}<b className="schedule-supplier-name">{ecosystemSupplierId ? `${sending ? "Sending" : "Receiving"} · ` : ""}{viewerName}</b><small className="schedule-booking-time">{shipment.scheduledTime}{pending ? " · not booked yet" : " · booked"}</small><span className="schedule-truck-plate">{pendingCompany ? "Company review" : pendingSupplier ? ecosystemSupplierId && !sending ? "Waiting for supplier" : "Waiting for confirmation" : shipment.truckPlate}</span></button>;
      })}
    </div>)}
  </div>;
}
...
export type SupplierResponsePayload = {
  ...
  trucks?: { truckPlate: string; driverName: string; driverPhone: string; helper1Name?: string; helper2Name?: string; poNumber?: string; drNumber?: string; itemIds: number[]; itemQuantities?: { itemId: number; quantity: number; poNumber?: string }[] }[];
};
...
function ScheduleEditModal({ shipment, onClose, onSubmit }: { shipment: Shipment; onClose: () => void; onSubmit: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void> }) {
  ...
  const save = async (event: React.FormEvent) => {
    event.preventDefault();
    const quantities = items.map(item => ({ id: item.id, quantity: Number(item.quantity) }));
    if (!date || !time || !endTime || minutes(endTime) <= minutes(time)) { setError("Choose an end time later than the entrance time."); return; }
    if (quantities.some(item => !Number.isFinite(item.quantity) || item.quantity < 0)) { setError("Use a quantity of zero or greater for every material."); return; }
    setBusy(true); setError("");
    try { await onSubmit(shipment, { scheduledDate: date, scheduledTime: time, scheduledEndTime: endTime, items: quantities }); onClose(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "The schedule could not be updated."); }
    finally { setBusy(false); }
  };
  return <Dialog title="Update delivery schedule" close={onClose}><form className="receiving-form schedule-edit-form" onSubmit={save}>
    <div className="confirmation-context"><b>{shipment.supplier}</b><span>{shipment.shipmentNumber}</span></div>
    <div className="schedule-edit-times"><label>Delivery date<input type="date" value={date} onChange={event => setDate(event.target.value)} required/></label><label>Entrance time<input type="time" value={time} onChange={event => setTime(event.target.value)} required/></label><label>End time<input type="time" value={endTime} onChange={event => setEndTime(event.target.value)} required/></label></div>
    <div className="schedule-edit-items"><h3>Material quantities</h3>{items.map((item, index) => <label key={item.id}><span><b>{item.code}</b><small>{item.uom}</small></span><input type="number" min="0" step="any" value={item.quantity} onChange={event => setItems(current => current.map((row, rowIndex) => rowIndex === index ? { ...row, quantity: event.target.value } : row))}/></label>)}</div>
    {error && <p className="form-error" role="alert">{error}</p>}<div className="confirmation-footer"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? <Loader2 className="spin" size={17}/> : <Check size={17}/>} Save updates</button></div>
  </form></Dialog>;
}

export function FlexibleSchedulePage({ data, user, area, canSwitchArea, ecosystemMode = false, unfilteredMode = false, onAreaChange, onOpenShipment, onImportSds, onRequestDelivery, onReviewAlternative, onUpdateSchedule, onDownloadSds }: { data: AppData; user: SessionUser; area: ReceivingArea; canSwitchArea: boolean; ecosystemMode?: boolean; unfilteredMode?: boolean; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void; onImportSds: () => void; onRequestDelivery: () => void; onReviewAlternative: (shipment: Shipment) => void; onUpdateSchedule: (shipment: Shipment, change: { scheduledDate: string; scheduledTime: string; scheduledEndTime: string; items: { id: number; quantity: number }[] }) => Promise<void>; onDownloadSds: (area: ReceivingArea) => Promise<void> }) {
  ...
  const areaData = useMemo(() => ({ ...data, shipments: ecosystemMode || unfilteredMode ? data.shipments : data.shipments.filter(shipment => shipmentArea(shipment) === area) }), [data, area, ecosystemMode, unfilteredMode]);
  const openSchedule = (shipment: Shipment) => canManage && !["GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED", "GATE_OUT", "REJECTED"].includes(shipment.status) ? setEditing(shipment) : onOpenShipment(shipment);
  ...
  const bookingCount = areaData.shipments.filter((shipment) => visibleDates.includes(shipment.scheduledDate) && shipment.bookingStatus === "APPROVED" && shipment.status !== "REJECTED").length;
  const period = mode === "day" ? formatDate(date) : `${formatDate(startOfWeek(date), true)} – ${formatDate(addDays(startOfWeek(date), 6), true)}`;
  const pendingSupplierBookings = ["supplier", "ecosystem"].includes(user.role) ? areaData.shipments.filter((shipment) => shipment.bookingStatus === "PENDING_SUPPLIER" && Number(shipment.supplierId) === Number(user.supplierId)).sort((a, b) => `${a.scheduledDate}${a.scheduledTime}`.localeCompare(`${b.scheduledDate}${b.scheduledTime}`)) : [];
  return <div className="page-stack schedule-page">
    {editing && <ScheduleEditModal shipment={editing} onClose={() => setEditing(null)} onSubmit={onUpdateSchedule}/>} 
    <section className="hero-row"><div><span className="eyebrow">Scheduling center</span><h1>Delivery schedule</h1></div><div className="hero-actions schedule-hero-actions">{canImport && <><button className="button secondary schedule-quick-action" onClick={() => void onDownloadSds(area)}><Download size={17}/> Updated SDS</button><button className="button secondary schedule-quick-action import-sds-button" onClick={onImportSds}><FileSpreadsheet size={17}/> Import SDS</button></>}{["admin", "planner", "supplier"].includes(user.role) && <button className="button primary" onClick={onRequestDelivery}><Plus size={17}/> Request</button>}{ecosystemMode ? <span className="area-scope-chip ecosystem-scope-chip">Receiving &amp; sending</span> : unfilteredMode ? <span className="area-scope-chip ecosystem-scope-chip receiving">Ecosystem receiving</span> : canSwitchArea ? <AreaToggle value={area} onChange={onAreaChange}/> : <span className="area-scope-chip">{area === "DRESSINGS" ? "Dressings" : "Savoury"}</span>}</div></section>
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      <div className="panel-head schedule-board-head"><div><span className="eyebrow">{period}</span><h2>Booked delivery schedule</h2></div><div className="schedule-view-controls schedule-command-bar"><span className="approved-delivery-count"><i/><b>{bookingCount}</b> Approved Deliveries</span>{canReviewCompany&&<SdsWorkflowPanel data={areaData} onReviewAlternative={onReviewAlternative}/>}<div className="view-toggle"><button className={mode === "day" ? "active" : ""} onClick={() => setMode("day")}>Day</button><button className={mode === "week" ? "active" : ""} onClick={() => setMode("week")}>Week</button></div><div className="date-stepper"><button type="button" onClick={() => move(-1)} aria-label={`Previous ${mode}`}><ArrowLeft size={17} /></button><CustomDatePicker value={date} onChange={setDate} label="Schedule date" /><button type="button" onClick={() => move(1)} aria-label={`Next ${mode}`}><ArrowRight size={17} /></button></div></div></div>
      <div className="supplier-legend">{ecosystemMode && <span className="ecosystem-direction-legend sending"><i style={{background:`hsl(${ECOSYSTEM_SENDING_HUE} 65% 43%)`}}/>Sending · Ecosystem → Company</span>}{Array.from(new Map(areaData.shipments.filter(row=>visibleDates.includes(row.scheduledDate) && (!ecosystemMode || !isEcosystemSending(row, user.supplierId))).map(row=>[row.supplierId||row.supplier,row])).values()).map(row=><span key={row.supplierId||row.supplier} className={ecosystemMode ? "ecosystem-direction-legend receiving" : ""}><i style={{background:`hsl(${colorFor(row)} 65% 48%)`}}/>{ecosystemMode ? `Receiving · ${row.supplier} → Ecosystem` : user.role === "supplier" ? `Cavite Foods ${shipmentArea(row) === "DRESSINGS" ? "Dressings" : "Savoury"}` : row.supplier}</span>)}</div>
      {mode === "day" && <div className="schedule-day-details">{areaData.shipments.filter(row=>row.scheduledDate===date && row.bookingStatus!=="REJECTED").sort((a,b)=>a.scheduledTime.localeCompare(b.scheduledTime)).map(row=>{const sending=isEcosystemSending(row,user.supplierId);return <button key={row.id} className={ecosystemMode ? sending ? "ecosystem-sending" : "ecosystem-receiving" : ""} style={{borderLeftColor:`hsl(${scheduleHue(row, ecosystemMode ? user.supplierId : null)} 65% 48%)`}} onClick={()=>openSchedule(row)}><b>{row.scheduledTime}</b><span><strong>{ecosystemMode ? sending ? "Ecosystem → Company" : `${row.supplier} → Ecosystem` : user.role === "supplier" ? `Cavite Foods ${shipmentArea(row) === "DRESSINGS" ? "Dressings" : "Savoury"}` : row.supplier}</strong><small>{row.items.map(item=>`${item.materialName || item.materialCode} · ${item.quantity} ${item.uom}`).join(" / ")}</small></span><span>{ecosystemMode && !sending && row.bookingStatus === "PENDING_SUPPLIER" ? "Waiting for supplier" : row.truckPlate||"Waiting for confirmation"}<small>{row.shipmentNumber}</small></span>{canManage && <em className="schedule-list-edit">Change</em>}</button>;})}</div>}
      <div className="schedule-timeline-viewport"><ScheduleTimeline shipments={areaData.shipments} anchorDate={date} mode={mode} showPending={["admin", "planner", "supplier", "ecosystem"].includes(user.role)} canManage={canManage} ecosystemSupplierId={ecosystemMode ? user.supplierId : null} onOpenShipment={openSchedule} /></div>
    </section>
  </div>;
}

export function MonitoringPage({ data, theme, area, canSwitchArea, ecosystemScope, onAreaChange, onOpenShipment }: { data: AppData; theme: "light" | "dark"; area: ReceivingArea; canSwitchArea: boolean; ecosystemScope?: EcosystemScope; onAreaChange: (value: ReceivingArea) => void; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  useEffect(() => { const timer = window.setInterval(() => setCurrentTime(Date.now()), 60000); return () => window.clearInterval(timer); }, []);
  ...
  const openMonitoringEntry = (shipment: Shipment) => onOpenShipment(shipment);
  const active = useMemo(() => data.shipments.filter((shipment) => (ecosystemScope || shipmentArea(shipment) === area) && shipment.bookingStatus === "APPROVED" && !["GATE_OUT", "REJECTED"].includes(shipment.status)), [data.shipments, area, ecosystemScope]);
  ...
  const receivingDocks = ["Dock 1 - PM", "Dock 2 - PM", "Dock 3 - RM"];
  const dockShipments = active.filter((shipment) => shipment.dock && ["GATE_IN", "SAP_CLEARANCE", "UNLOADING", "RECEIVED"].includes(shipment.status));
  const waitingAtGate = active.filter((shipment) => shipment.status === "GATE_IN" && !shipment.dock);
  return <div ref={fullscreenRef} className={`page-stack monitoring-page tv-${theme} ${fullscreen ? "tv-mode" : ""}`}>
    {!fullscreen && <section className="hero-row"><div><span className="eyebrow">Truck movement board</span><h1>{ecosystemScope === "receiving" ? "Receiving monitoring" : ecosystemScope === "sending" ? "Sending monitoring" : "Delivery monitoring"}</h1></div><div className="hero-actions">{ecosystemScope ? <span className={`area-scope-chip ecosystem-scope-chip ${ecosystemScope}`}>{ecosystemScope === "receiving" ? "Supplier → Ecosystem" : "Ecosystem → Company"}</span> : canSwitchArea ? <AreaToggle value={area} onChange={onAreaChange}/> : <span className="area-scope-chip">{area === "DRESSINGS" ? "Dressings" : "Savoury"}</span>}<span className="operation-live"><span className="live-dot" /><span>Live status</span></span><button className="icon-button fullscreen-trigger" onClick={enterFullscreen} aria-label="Open fullscreen monitoring" title="Fullscreen"><Maximize2 size={19} /></button></div></section>}
    {fullscreen && <header className="monitor-tv-header">
      <div className="monitor-tv-brand"><span className="eyebrow">Live {ecosystemScope || "receiving"} board</span><h1>{data.settings.siteName}</h1><span className="monitor-tv-live"><i className="live-dot" />{rows.length} active deliver{rows.length === 1 ? "y" : "ies"}</span></div>
    ...
    <div className={fullscreen ? "monitor-tv-layout" : undefined}>
      ...
      <div className="monitor-grid">{rows.map((shipment) => {
        ...
        const trafficSuffix = timeOfDayTraffic
          ? shipment.estimatedTrafficDelayMinutes
            ? ` · includes ${shipment.estimatedTrafficDelayMinutes} min traffic`
            : " · traffic-adjusted"
          : " · base road time";
        ...
        const estimatedAt = Date.parse(shipment.estimatedArrivalAt || "");
        const etaVarianceMinutes = Number.isFinite(scheduledAt) && Number.isFinite(estimatedAt) ? Math.round((estimatedAt - scheduledAt) / 60000) : null;
        const late = shipment.arrivalClassification === "LATE" || (shipment.status === "IN_TRANSIT" && etaVarianceMinutes !== null && etaVarianceMinutes > 15) || (["BOOKED", "IN_TRANSIT"].includes(shipment.status) && currentTime > scheduledAt + 15 * 60000);
        const onTime = ["ADVANCED", "ON_TIME"].includes(shipment.arrivalClassification || "") || (shipment.status === "IN_TRANSIT" && etaVarianceMinutes !== null && etaVarianceMinutes <= 15);
        const timingTone = late ? "red" : onTime ? "green" : "blue";
        const etaTiming = etaVarianceMinutes == null ? "Trip-based ETA" : Math.abs(etaVarianceMinutes) <= 1 ? "Trip-based ETA · on schedule" : `Trip-based ETA · ${durationLabel(etaVarianceMinutes)} ${etaVarianceMinutes > 0 ? "late" : "early"}`;
        const etaDetails = showEta && shipment.estimatedArrivalAt ? `${formatEta(shipment.estimatedArrivalAt)} · ${shipment.estimatedTravelMinutes} min total · ${shipment.estimatedTravelDistanceKm || "—"} km${trafficSuffix}` : "ETA pending";
        return <button className={`monitor-delivery-card monitor-tone-${timingTone} ${ecosystemScope ? `ecosystem-${ecosystemScope}` : ""}`} key={shipment.id} onClick={() => openMonitoringEntry(shipment)}>
          <span className="monitor-card-head">
            <span>{ecosystemScope && <em className={`monitor-direction-chip ${ecosystemScope}`}>{ecosystemScope}</em>}<small>Scheduled · {formatDate(shipment.scheduledDate)}</small><b>{shipment.scheduledTime}</b></span>
            <span className="monitor-card-meta">
              <span className={`monitor-eta-top ${showEta ? "" : "hidden"}`} aria-hidden={!showEta} title={showEta ? etaDetails : undefined}><Clock3 size={14} /><span><small>{etaTiming}</small><b>{etaDetails}</b></span></span>
          ...
          <span className="monitor-card-shipment"><span className="truck-tile"><Truck size={19} /></span><span><b>{shipment.supplier}</b><small>{shipment.truckPlate}</small></span><ArrowRight size={17} /></span>
          ...
          <span className="monitor-card-material"><b>{shipment.items.map((item) => item.materialName || item.materialCode).join(", ") || "Materials pending"}</b><small>{shipment.items.map((item) => `${item.quantity.toLocaleString()} ${item.uom}`).join(" · ")}</small></span>
...
export function HistoryPage({ data, onOpenShipment }: { data: AppData; user: SessionUser; onOpenShipment: (shipment: Shipment) => void }) {
  ...
  const received = useMemo(() => data.shipments.filter((shipment) => shipment.status === "GATE_OUT").sort((a, b) => String(b.gateOutAt || "").localeCompare(String(a.gateOutAt || ""))), [data.shipments]);
