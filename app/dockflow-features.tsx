...
function SupplierSdsModal({ shipment, onClose, onSubmit }: { shipment: Shipment; onClose: () => void; onSubmit: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void }) {
  ...
  const [alternativeEndTime, setAlternativeEndTime] = useState(shipment.alternativeEndTime || shipment.scheduledEndTime || shipment.scheduledTime);
  const [truck, setTruck] = useState({ truckPlate: "", driverName: "", driverPhone: "" });
  const [selectedItems, setSelectedItems] = useState<number[]>([]);
  const [confirmed, setConfirmed] = useState(false), [busy, setBusy] = useState(false);
  const updateTruck = (field: "truckPlate" | "driverName" | "driverPhone", value: string) => setTruck((current) => ({ ...current, [field]: field === "driverPhone" ? value.replace(/\D/g, "") : value }));
  ...
  const valid = Boolean(confirmed && validAlternative && truck.truckPlate.trim() && selectedItems.length > 0);
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      await onSubmit(shipment, {
        ...
        trucks: [{ ...truck, truckPlate: truck.truckPlate.trim().toUpperCase(), itemIds: selectedItems }],
      });
      onClose();
    } finally { setBusy(false); }
  };
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal modal-wide sds-response-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><h2>Confirm a truck load</h2><p>{formatDate(shipment.scheduledDate, true)} · {shipment.scheduledTime}–{shipment.scheduledEndTime || shipment.scheduledTime} · {remainingItems.length} material code{remainingItems.length === 1 ? "" : "s"} remaining</p></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div><form onSubmit={submit}>
    <div className="sds-response-body">{firstLoad ? <><div className="sds-choice-row"><button type="button" className={decision === "ACCEPT" ? "active" : ""} onClick={() => setDecision("ACCEPT")}><Check size={17} /><span><b>Accept proposed time</b><small>Use the SDS date and time</small></span></button><button type="button" className={decision === "PROPOSE_ALTERNATIVE" ? "active warning" : ""} onClick={() => setDecision("PROPOSE_ALTERNATIVE")}><CalendarDays size={17} /><span><b>Reject and propose alternative</b><small>Reason and one alternative are required</small></span></button></div>
      {decision === "PROPOSE_ALTERNATIVE" && <div className="sds-alternative-grid"><label>Reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why the proposed time cannot be used" required /></label><label>Alternative date<input type="date" value={alternativeDate} onChange={(event) => setAlternativeDate(event.target.value)} required /></label><label>From<input type="time" value={alternativeTime} onChange={(event) => setAlternativeTime(event.target.value)} required /></label><label>To<input type="time" value={alternativeEndTime} onChange={(event) => setAlternativeEndTime(event.target.value)} required /></label></div>}</> : <div className="saved-response-note"><Check size={17} /><span><b>Schedule response saved</b>{shipment.supplierResponse === "ALTERNATIVE_PROPOSED" ? `${shipment.alternativeDate} · ${shipment.alternativeTime}–${shipment.alternativeEndTime}` : "Proposed date and time accepted"}</span></div>}
      {previousLoads.length > 0 && <div className="confirmed-load-list">{previousLoads.map((load) => <span key={load.id}><b>{load.truckPlate}</b><small>{load.deliveryCode} · {load.itemIds.length} code{load.itemIds.length === 1 ? "" : "s"}</small></span>)}</div>}
      <div className="truck-load-head"><div><span className="eyebrow">Truck details</span><h3>Confirm one truck at a time</h3></div></div>
      <div className="truck-load-grid single"><article><header><span>Next truck</span></header><label>Truck plate<input value={truck.truckPlate} onChange={(event) => updateTruck("truckPlate", event.target.value)} placeholder="ABC 1234" required /></label><label>Driver name<input value={truck.driverName} onChange={(event) => updateTruck("driverName", event.target.value)} placeholder="Driver name" /></label><label>Phone number<input inputMode="numeric" pattern="[0-9]*" value={truck.driverPhone} onChange={(event) => updateTruck("driverPhone", event.target.value)} placeholder="Digits only" /></label></article></div>
      <div className="material-allocation selectable"><div className="panel-head"><div><span className="eyebrow">Material allocation</span><h3>Select every code carried by this truck</h3></div><span className="count-chip">{selectedItems.length} selected</span></div>{remainingItems.map((item) => <label key={item.id}><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={() => setSelectedItems((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /><span><b>{item.materialCode}</b><small>{item.quantity.toLocaleString()} {item.uom}</small></span></label>)}</div>
      <label className="load-confirm-check"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span><b>I confirm this truck and its selected material codes.</b><small>A delivery code is reserved now. Return to confirm another truck if materials remain.</small></span></label>
    </div><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={!valid || busy}>{busy ? <Loader2 className="spin" size={17} /> : <Check size={17} />} Confirm this truck</button></div>
  </form></section></div>;
}
...
function SdsWorkflowPanel({ data, user, onImportSds, onSupplierResponse, onFinalDecision }: { data: AppData; user: SessionUser; onImportSds: () => void; onSupplierResponse: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void; onFinalDecision: (shipment: Shipment, decision: "APPROVE" | "REJECT", reason?: string) => Promise<void> | void }) {
  ...
  const rows = user.role === "supplier" ? supplierPending : [...unlinked, ...ready];
  return <section className="panel sds-workflow-panel"><div className="panel-head"><div><span className="eyebrow">SDS workflow</span><h2>{user.role === "supplier" ? "Confirm proposed deliveries" : "Final delivery decisions"}</h2></div>{planning && <button className="button primary" onClick={onImportSds}><FileSpreadsheet size={17} /> Import SDS</button>}</div>
    <div className="sds-workflow-list">{rows.map((shipment) => { const group = data.shipments.filter((row) => Number(row.sdsProposalId || row.id) === Number(shipment.sdsProposalId || shipment.id)); const site = shipment.items.find((item) => item.deliverySite)?.deliverySite || "Site not supplied"; const week = shipment.items.find((item) => item.deliveryWeek)?.deliveryWeek; const remaining = shipment.items.filter((item) => !item.supplierApprovedAt).length; const unlinkedAccount = shipment.supplierAccountLinked === false; return <article className={unlinkedAccount ? "account-unlinked" : ""} key={shipment.id}><span className="truck-tile"><Truck size={19} /></span><div className="review-main"><strong>{shipment.supplier}</strong><small>{site}{week ? ` · Week ${week}` : ""}</small><span>{group.flatMap((row) => row.items).map((item) => `${item.materialCode}: ${item.quantity.toLocaleString()} ${item.uom}`).join(" · ")}</span>{unlinkedAccount && <span className="account-missing-label"><AlertTriangle size={13} /> No supplier account</span>}</div><div className="review-schedule"><strong>{formatDate(shipment.scheduledDate, true)}</strong><small>{shipment.scheduledTime}–{shipment.scheduledEndTime || shipment.scheduledTime}</small>{shipment.bookingStatus === "SUPPLIER_ALTERNATIVE" && <span>Alternative: {shipment.alternativeDate} · {shipment.alternativeTime}–{shipment.alternativeEndTime}</span>}{user.role === "supplier" && <span>{remaining} material code{remaining === 1 ? "" : "s"} remaining</span>}{planning && !unlinkedAccount && <span>{group.length} truck{group.length === 1 ? "" : "s"}</span>}</div>{user.role === "supplier" ? <button className="button primary compact" onClick={() => setResponding(shipment)}>{shipment.confirmedTruckLoads?.length ? "Confirm next truck" : "Review & confirm"}</button> : unlinkedAccount ? <span className="approval-chip pending">Account required</span> : rejecting === shipment.id ? <div className="review-reject"><textarea autoFocus value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Final rejection reason" /><div><button className="button secondary compact" onClick={() => { setRejecting(null); setReason(""); }}>Cancel</button><button className="button danger compact" disabled={!reason.trim()} onClick={async () => { await onFinalDecision(shipment, "REJECT", reason.trim()); setRejecting(null); setReason(""); }}>Reject</button></div></div> : <div className="management-row-actions"><button className="button primary compact" onClick={() => void onFinalDecision(shipment, "APPROVE")}><Check size={14} /> Final approve</button><button className="button secondary compact" onClick={() => setRejecting(shipment.id)}><X size={14} /> Reject</button></div>}</article>; })}</div>
...
export function FlexibleSchedulePage({ data, user, onOpenShipment, onSaveAvailability, onDeleteAvailability, onMoveShipment, onImportSds, onSupplierResponse, onFinalDecision }: { data: AppData; user: SessionUser; onOpenShipment: (shipment: Shipment) => void; onSaveAvailability: (slot: AvailabilityInput) => Promise<void> | void; onDeleteAvailability: (id: number) => Promise<void> | void; onMoveShipment: (shipment: Shipment, date: string, startTime: string, endTime: string) => Promise<void> | void; onImportSds: () => void; onSupplierResponse: (shipment: Shipment, payload: SupplierResponsePayload) => Promise<void> | void; onFinalDecision: (shipment: Shipment, decision: "APPROVE" | "REJECT", reason?: string) => Promise<void> | void }) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel schedule-board overlap-schedule schedule-section-panel unified">
      <div className="panel-head"><div><span className="eyebrow">{period}</span><h2>{canEdit ? "Availability and approved deliveries" : "Times available to book"}</h2></div><span className="policy-chip"><Clock3 size={14} /> Manila time · GMT+8</span></div>
